...
wxBoxSizer *TipsDialog::create_item_checkbox(wxString title, wxWindow *parent, wxString tooltip, std::string param)
{
    ...
    m_show_again = app_get_bool(param);
...
void TipsDialog::on_ok(wxMouseEvent &event)
{
    if (m_show_again) {
        if (!m_app_key.empty())
        app_set_bool(m_app_key, m_show_again);
    }
    EndModal(wxID_OK);
}
    ...
    : wxPanel( parent, id, pos, size, style, name )
{
    ...
    if (dynamic_cast<Notebook*>(parent)) {
        ...
        m_title_view = new Label(m_top_panel, Font::Body_12, _L("Advance")); // ORCA match size with advanced toggle on tab.cpp m_static_title
        ...
        //m_search_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { AppAdapter::plater()->search(false); });
        ...
        m_compare_btn->Bind(wxEVT_BUTTON, ([this](wxCommandEvent e) { AppAdapter::main_panel()->diff_dialog.show(); }));
        ...
        m_setting_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { AppAdapter::plater()->PopupObjectTable(-1, -1, {0, 0}); });
    ...
    //Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { AppAdapter::plater()->search(false); }, wxID_FIND);
    //m_export_to_file->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { AppAdapter::main_panel()->export_config(); });
    //m_import_from_file->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { AppAdapter::main_panel()->load_config_file(); });
}
...
void ParamsPanel::refresh_tabs()
{
    ...
    if (m_top_panel) {
        m_tab_print_plate = AppAdapter::gui_app()->get_plate_tab();
        m_tab_print_object = AppAdapter::gui_app()->get_model_tab();
        m_tab_print_part = AppAdapter::gui_app()->get_model_tab(true);
        m_tab_print_layer = AppAdapter::gui_app()->get_layer_tab();
    }
    return;
}
