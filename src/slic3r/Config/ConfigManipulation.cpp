...
void ConfigManipulation::check_chamber_temperature(DynamicPrintConfig* config)
{
   ...
   bool support_chamber_temp_control=GUI::app_preset_bundle()->printers.get_selected_preset().config.opt_bool("support_chamber_temp_control");
...
void ConfigManipulation::update_print_fff_config(DynamicPrintConfig* config, const bool is_global_config, const bool is_plate_config)
{
    ...
    auto gpreset = GUI::app_preset_bundle()->printers.get_edited_preset();
    ...
    // BBS
    int filament_cnt = app_preset_bundle()->filament_presets.size();
#if 0
