...
PresetUpdater::priv::priv()
	: cache_path(fs::path(Slic3r::user_data_dir()) / "ota")
	, rsrc_path(fs::path(resources_dir()) / "profiles")
	, vendor_path(fs::path(Slic3r::user_data_dir()) / PRESET_SYSTEM_DIR)
	, cancel(false)
{
	//BBS: refine preset updater logic
	enabled_version_check = true;
	set_download_prefs(GUI::AppAdapter::app_config());
    ...
    for (auto & resource_it : resources) {
        ...
        if (version_match && (current_version < online_version)) {
            ...
            try {
                ...
                if (!c.is_open()) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": failed to open file " << changelog_file;
                    break;
                }
...
void PresetUpdater::priv::sync_config()
{
    ...
    AppConfig *app_config = GUI::AppAdapter::app_config();

    auto profile_update_url = app_config->profile_update_url() + "/" + LightMaker_VERSION;
...
void PresetUpdater::priv::sync_tooltip(std::string http_url, std::string language)
{
    try {
        ...
        fs::path cache_root = fs::path(user_data_dir()) / "resources/tooltip";
        try {
...
void PresetUpdater::priv::sync_plugins(std::string http_url, std::string plugin_version)
{
    ...
    if (result) {
        ...
        if (force_upgrade) {
            auto app_config = GUI::AppAdapter::app_config();
            if (!app_config)
                GUI::AppAdapter::plater()->get_notification_manager()->push_notification(GUI::NotificationType::BBLPluginUpdateAvailable);
            else
...
void PresetUpdater::priv::sync_printer_config(std::string http_url)
{
    ...
    if (result) {
        ...
        if (waiting_printer_updates.updates.size() > 0) {
            ...
            GUI::AppAdapter::plater()->get_notification_manager()->push_notification(GUI::NotificationType::BBLPrinterConfigUpdateAvailable);
        }
    }
}
...
bool PresetUpdater::priv::perform_updates(Updates &&updates, bool snapshot) const
{
    ...
    if (updates.incompats.size() > 0) {
        ...
        //	if (! GUI::Config::take_config_snapshot_cancel_on_error(*GUI::AppAdapter::app_config(), Snapshot::SNAPSHOT_DOWNGRADE, "",
        ...
        //if (snapshot) {
        //	BOOST_LOG_TRIVIAL(info) << "Taking a snapshot...";
        //	if (! GUI::Config::take_config_snapshot_cancel_on_error(*GUI::AppAdapter::app_config(), Snapshot::SNAPSHOT_UPGRADE, "",
        //		_u8L("Continue and install configuration updates?")))
        //		return false;
        //}
...
static bool reload_configs_update_gui()
{
	...
	if (!GUI::AppAdapter::gui_app()->check_and_save_current_preset_changes(_L("Configuration updates"), header, false ))
		return false;
	...
	GUI::app_preset_bundle()->load_presets(*app_config, ForwardCompatibilitySubstitutionRule::EnableSilentDisableSystem);
	GUI::AppAdapter::gui_app()->load_current_presets();
	GUI::AppAdapter::plater()->set_bed_shape();
