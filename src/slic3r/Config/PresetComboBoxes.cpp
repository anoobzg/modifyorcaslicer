...
PresetComboBox::PresetComboBox(wxWindow* parent, Preset::Type preset_type, const wxSize& size, PresetBundle* preset_bundle/* = nullptr*/) :
    ...
    m_preset_bundle(preset_bundle ? preset_bundle : app_preset_bundle())
{
#ifdef __WXMSW__
    if (preset_type == Preset::TYPE_FILAMENT)
        SetFont(Font::Body_13);
#endif // __WXMSW__
...
int PresetComboBox::update_ams_color()
{
    ...
    if (idx < 0) {
        ...
        auto &ams_list = app_preset_bundle()->filament_ams_list;
    ...
    DynamicPrintConfig *cfg        = &app_preset_bundle()->project_config;
    ...
    cfg->apply(new_cfg);
    AppAdapter::plater()->on_config_change(new_cfg);
...
bool PresetComboBox::selection_is_changed_according_to_physical_printers()
{
    ...
    if (old_printer_preset == preset_name) {
        ...
        if (dynamic_cast<PlaterPresetComboBox*>(this)!=nullptr) {
            AppAdapter::gui_app()->get_tab(m_type)->update_preset_choice();
            // Synchronize config.ini with the current selections.
            m_preset_bundle->export_selections(*AppAdapter::app_config());
        }
        else if (dynamic_cast<TabPresetComboBox*>(this)!=nullptr)
            AppAdapter::gui_app()->sidebar().update_presets(m_type);
...
PlaterPresetComboBox::PlaterPresetComboBox(wxWindow *parent, Preset::Type preset_type) :
    PresetComboBox(parent, preset_type, wxSize(25 * app_em_unit(), 30 * app_em_unit() / 10))
{
    ...
    if (m_type == Preset::TYPE_FILAMENT) {
        ...
        clr_picker->Bind(wxEVT_BUTTON, [this](wxCommandEvent& e) {
            ...
            std::vector<std::string> colors = AppAdapter::app_config()->get_custom_color_from_config();
            ...
            if ( dialog.ShowModal() == wxID_OK )
            {
                ...
                update();
                AppAdapter::plater()->on_config_change(cfg_new);
...
{
    AppAdapter::gui_app()->run_wizard(ConfigWizard::RR_USER, sp);
}
...
{
    Tab* tab = AppAdapter::gui_app()->get_tab(m_type);
    if (!tab)
        return false;
    ...
    if (m_type == Preset::TYPE_FILAMENT)
    {
        ...
        {
            const std::string& preset_name = app_preset_bundle()->filaments.get_preset_name_by_alias(selected_preset);
            if (AppAdapter::gui_app()->get_tab(m_type)->select_preset(preset_name))
                AppAdapter::gui_app()->get_tab(m_type)->get_combo_box()->set_filament_idx(m_filament_idx);
            else {
                AppAdapter::gui_app()->params_dialog()->Hide();
                return false;
            }
        }
    }
    ...
    /*
    if (int page_id = AppAdapter::gui_app()->tab_panel()->FindPage(tab); page_id != wxNOT_FOUND)
    {
        AppAdapter::gui_app()->tab_panel()->SetSelection(page_id);
        // Switch to Settings NotePad
        AppAdapter::main_panel()->select_tab();

        //In a case of a multi-material printing, for editing another Filament Preset
        //it's needed to select this preset for the "Filament settings" Tab
        if (m_type == Preset::TYPE_FILAMENT && app_extruders_edited_cnt() > 1)
        {
            const std::string& selected_preset = GetString(GetSelection()).ToUTF8().data();
            ...
            {
                const std::string& preset_name = app_preset_bundle()->filaments.get_preset_name_by_alias(selected_preset);
                AppAdapter::gui_app()->get_tab(m_type)->select_preset(preset_name);
            }
        }
    }
    */
...
void PlaterPresetComboBox::change_extruder_color()
{
    ...
    if (dialog.ShowModal() == wxID_OK)
    {
        ...
        AppAdapter::gui_app()->get_tab(Preset::TYPE_PRINTER)->load_config(cfg_new);
        this->update();
        AppAdapter::plater()->on_config_change(cfg_new);
    }
}
...
void PlaterPresetComboBox::show_add_menu()
{
    ...
    append_menu_item(menu, wxID_ANY, _L("Add/Remove presets"), "",
        ...
        }, "menu_edit_preset", menu, []() { return true; }, AppAdapter::plater());

    AppAdapter::plater()->PopupMenu(menu);
}
...
void PlaterPresetComboBox::show_edit_menu()
{
    ...
    append_menu_item(menu, wxID_ANY, _L("Edit preset"), "",
        [this](wxCommandEvent&) { this->switch_to_tab(); }, "cog", menu, []() { return true; }, AppAdapter::plater());

#ifdef __linux__
    // To edit extruder color from the sidebar
    if (m_type == Preset::TYPE_FILAMENT) {
        append_menu_item(menu, wxID_ANY, _devL("Change extruder color"), "",
            [this](wxCommandEvent&) { this->change_extruder_color(); }, "blank_14", menu, []() { return true; }, AppAdapter::plater());
        AppAdapter::plater()->PopupMenu(menu);
        return;
    }
#endif //__linux__
...
TabPresetComboBox::TabPresetComboBox(wxWindow* parent, Preset::Type preset_type) :
    // BBS: new layout
    PresetComboBox(parent, preset_type, wxSize(20 * app_em_unit(), 30 * app_em_unit() / 10))
{
}
