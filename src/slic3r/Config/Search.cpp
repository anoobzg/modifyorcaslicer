...
#include "libslic3r/FileSystem/fts_fuzzy_match.h"
...
SearchItem::SearchItem(wxWindow *parent, wxString text, int index, SearchDialog* sdialog, SearchObjectDialog* search_dialog, wxString tooltip)
    : wxWindow(parent, wxID_ANY, wxDefaultPosition, wxSize(parent->GetSize().GetWidth(), 3 * GUI::app_em_unit()))
{
    m_sdialog = sdialog;
...
{
    if (bold) {
        dc.SetFont(Font::Head_14);
    } else {
        dc.SetFont(Font::Body_14);
    }
...
void SearchItem::on_mouse_left_up(wxMouseEvent &evt)
{
    ...
    if (m_sdialog) {
        ...
        wxPostEvent(GUI::AppAdapter::plater(), event);
    }
    ...
    : PopupWindow(parent, wxBORDER_NONE | wxPU_CONTAINS_CONTROLS), searcher(searcher)
{
    ...
    search_line->SetFont(GUI::app_bold_font());
#endif
...
void SearchDialog::ProcessSelection(wxDataViewItem selection)
{
    ...
    // If call GUI::AppAdapter::gui_app()->sidebar.jump_to_option() directly from here,
