...
Field::Field(const ConfigOptionDef& opt, const t_config_option_key& id)
 : Field(nullptr, opt, id)
{

}

Field::Field(wxWindow* parent, const ConfigOptionDef& opt, const t_config_option_key& id)
...
#endif /* __APPLE__ */
                case 'F': {
                    //AppAdapter::plater()->search(false, Preset::TYPE_MODEL, nullptr, nullptr);
                    break;
                }
			    default: break;
			    }
			    if (tab_id >= 0)
					AppAdapter::main_panel()->select_tab(tab_id);
				if (tab_id > 0)
					// tab panel should be focused for correct navigation between tabs
				    AppAdapter::gui_app()->tab_panel()->SetFocus();
		    }
...
wxString Field::get_tooltip_text(const wxString &default_string)
{
    ...
    Slic3r::GUI::edit_tooltip(tooltip);
...
void free_window(wxWindow *win)
{
    ...
    if (auto sizer = win->GetContainingSizer())
        sizer->Clear();
    win->Reparent(AppAdapter::main_panel());
...
void CheckBox::BUILD() {
	...
	// BBS: use CheckUIBox
    static Builder<CheckUIBox> builder;
...
{
	m_disable_change_event = !change_event;
    dynamic_cast<CheckUIBox *>(window)->SetValue(value); // BBS
	m_disable_change_event = false;
}
...
void CheckBox::set_value(const boost::any& value, bool change_event)
{
    ...
    if (m_opt.nullable) {
        ...
        dynamic_cast<CheckUIBox*>(window)->SetValue(m_is_na_val ? false : bool_value); // BBS
    }
    else if (!value.empty()) // BBS: null value
        dynamic_cast<CheckUIBox*>(window)->SetValue(boost::any_cast<bool>(value)); // BBS
    dynamic_cast<CheckUIBox*>(window)->SetHalfChecked(value.empty());
    m_disable_change_event = false;
}
...
void CheckBox::set_last_meaningful_value()
{
    if (m_opt.nullable) {
        m_is_na_val = false;
        dynamic_cast<CheckUIBox*>(window)->SetValue(boost::any_cast<unsigned char>(m_last_meaningful_value) != 0); // BBS
        on_change_field();
    }
}
...
void CheckBox::set_na_value()
{
    if (m_opt.nullable) {
        m_is_na_val = true;
        dynamic_cast<CheckUIBox*>(window)->SetValue(false); // BBS
        on_change_field();
    }
}
...
{
// 	boost::any m_value;
	bool value = dynamic_cast<CheckUIBox*>(window)->GetValue(); // BBS
...
void CheckBox::msw_rescale()
{
	...
	// BBS: new param style
	CheckUIBox* field = dynamic_cast<CheckUIBox*>(window);
...
#endif //__WXGTK3__
	// temp->SetFont(Slic3r::GUI::app_normal_font()); // BBS
    if (!wxOSX) temp->SetBackgroundStyle(wxBG_STYLE_PAINT);
	UpdateDarkUI(temp);
...
void Choice::BUILD()
{
	...
	//temp->SetFont(Slic3r::GUI::app_normal_font());
    if (!wxOSX) temp->SetBackgroundStyle(wxBG_STYLE_PAINT);
...
void ColourPicker::BUILD()
{
	...
	UpdateDarkUI(temp->GetPickerCtrl());
...
void ColourPicker::sys_color_changed()
{
	...
	if (wxWindow* win = this->getWindow())
		if (wxColourPickerCtrl* picker = dynamic_cast<wxColourPickerCtrl*>(win))
			UpdateDarkUI(picker->GetPickerCtrl(), true);
#endif
}
...
void ColourPicker::on_button_click(wxCommandEvent &event) {
    ...
    if (m_clrData) {
        std::vector<std::string> colors = app_get_custom_color();
...
//#endif
    ...
    x_input->SetFont(app_normal_font());
    x_input->SetBackgroundStyle(wxBG_STYLE_PAINT);
    y_input->SetFont(app_normal_font());
	...
	//static_text_x->SetFont(Slic3r::GUI::app_normal_font());
	//static_text_x->SetBackgroundStyle(wxBG_STYLE_PAINT);
	//static_text_y->SetFont(Slic3r::GUI::app_normal_font());
	//static_text_y->SetBackgroundStyle(wxBG_STYLE_PAINT);

	UpdateDarkUI(x_input);
	UpdateDarkUI(y_input);
	//UpdateDarkUI(static_text_x, false, true);
	//UpdateDarkUI(static_text_y, false, true);
...
void SliderCtrl::BUILD()
{
	...
	m_slider->SetFont(app_normal_font());
	...
	m_textctrl->SetFont(app_normal_font());
