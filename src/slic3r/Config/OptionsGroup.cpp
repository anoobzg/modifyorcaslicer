...
void OptionsGroup::activate_line(Line& line)
{
    ...
    if (line.full_width && (
        ...
        if (!line.get_extra_widgets().empty()) {
            ...
            for (auto extra_widget : line.get_extra_widgets()) {
                h_sizer->Add(extra_widget(this->ctrl_parent()), is_first_item ? 1 : 0, wxLEFT, titleWidth * app_em_unit());
                is_first_item = false;
            }
            return;
        }
    ...
    if (option_set.size() == 1 && label_width == 0 && option_set.front().opt.full_width &&
		...
		if (is_window_field(field))
			h_sizer->Add(field->getWindow(), 1, wxEXPAND | wxLEFT, option.opt.multiline ? 0 : titleWidth * app_em_unit());
		if (is_sizer_field(field))
			h_sizer->Add(field->getSizer(), 1, wxEXPAND | wxLEFT, titleWidth * app_em_unit());
		return;
	}
        ...
        if (custom_ctrl) {
            ...
            if (!line.near_label_widget || !line.label.IsEmpty()) {
                ...
                label = new wxStaticText(this->ctrl_parent(), wxID_ANY, line.label + (line.label.IsEmpty() ? "" : ": "),
                    wxDefaultPosition, wxSize(label_width * app_em_unit(), -1), label_style);
                label->SetBackgroundStyle(wxBG_STYLE_PAINT);
                label->SetFont(app_normal_font());
                label->Wrap(label_width * app_em_unit()); // avoid a Linux/GTK bug
            }
            if (!line.near_label_widget)
...
//!			To correct translation by context have to use wxGETTEXT_IN_CONTEXT macro from wxWidget 3.1.1
			...
			label = new wxStaticText(this->ctrl_parent(), wxID_ANY, str_label + ": ", wxDefaultPosition, //wxDefaultSize);
				wxSize(sublabel_width != -1 ? sublabel_width * app_em_unit() : -1, -1), wxALIGN_RIGHT);
			label->SetBackgroundStyle(wxBG_STYLE_PAINT);
            label->SetFont(app_normal_font());
        ...
        if (!custom_ctrl) {
            ...
            if (!field->combine_side_text() && (!option.sidetext.empty() || sidetext_width > 0)) {
                auto sidetext = new wxStaticText(this->ctrl_parent(), wxID_ANY, _(option.sidetext), wxDefaultPosition,
                    wxSize(sidetext_width != -1 ? sidetext_width * app_em_unit() : -1, -1), wxALIGN_LEFT);
                sidetext->SetBackgroundStyle(wxBG_STYLE_PAINT);
                sidetext->SetFont(app_normal_font());
...
bool OptionsGroup::activate(std::function<void()> throw_if_canceled/* = [](){}*/, int horiz_alignment/* = wxALIGN_LEFT*/)
{
	...
	try {
		if (staticbox) {
			...
			stb->SetFont(wxOSX ? app_normal_font() : app_bold_font());
			UpdateDarkUI(stb);
			// BBS: new layout
...
Option ConfigOptionsGroup::get_option(const std::string& opt_key, int opt_index /*= -1*/)
{
	...
	if (m_use_custom_ctrl) // fill group and category values just for options from Settings Tab
	    AppAdapter::gui_app()->sidebar().get_searcher().add_key(opt_id, static_cast<Preset::Type>(this->config_type()), title, this->config_category());
...
#ifdef _WIN32
    if (staticbox && stb) {
        UpdateAllStaticTextDarkUI(stb);
        // update bitmaps for extra column items (like "delete" buttons on settings panel)
        for (auto extra_col : m_extra_column_item_ptrs)
            UpdateDarkUI(extra_col);
    }

    if (custom_ctrl)
        UpdateDarkUI(custom_ctrl);
#endif
    ...
    auto update = [](wxSizer* sizer) {
        for (wxSizerItem* item : sizer->GetChildren())
            if (item->IsWindow()) {
                ...
                UpdateDarkUI(win, dynamic_cast<wxButton*>(win) != nullptr);
            }
    };
