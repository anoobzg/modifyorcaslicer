...
#include "slic3r/GUI/Event/UserDriveDataEvent.hpp"
...
std::unique_ptr <GUI::OtherInstanceMessageHandler> m_other_instance_message_handler;
...
namespace instance_check_internal
{
	...
	static CommandLineAnalysis process_command_line(int argc, char** argv)
	{
		...
		BOOST_LOG_TRIVIAL(debug) << "Instance check: single instance: " << 
	...
	static BOOL CALLBACK EnumWindowsProc(_In_ HWND   hwnd, _In_ LPARAM lParam)
	{
		...
		if (wndTextString.find(L"OrcaSlicer") != std::wstring::npos && classNameString == L"wxWindowNR") {
			...
			uint64_t my_instance_hash = m_instance_hash_int;
	...
	static void delete_lockfile()
	{
		...
		{
			std::string path = user_data_dir() + "/cache/" + GUI::AppAdapter::gui_app()->get_instance_hash_string() + ".lock";
			if( remove( path.c_str() ) != 0 )
	   			BOOST_LOG_TRIVIAL(error) << "Failed to delete lockfile " << path;
	  		//else
	    	//	BOOST_LOG_TRIVIAL(error) << "success delete lockfile " << path;
#ifdef __APPLE__
	   		send_message_mac_closing(GUI::AppAdapter::gui_app()->get_instance_hash_string(),GUI::AppAdapter::gui_app()->get_instance_hash_string());
#endif	    
		}
	}
...
{
	m_other_instance_message_handler = std::make_unique<GUI::OtherInstanceMessageHandler>();
	...
	m_instance_hash_int = hashed_path; 
	m_instance_hash_string = std::to_string(hashed_path);

	BOOST_LOG_TRIVIAL(debug) <<"Instance check: Full hash path: "<< lock_name;
...
#ifdef _WIN32
	std::string checker_name = lock_name + ".lock";
	std::string checker_path = user_data_dir() + "\\cache\\";
    BOOST_LOG_TRIVIAL(debug) << "init wx instance checker " << checker_name << " "<< checker_path;
    m_single_instance_checker = std::make_unique<wxSingleInstanceChecker>(boost::nowide::widen(checker_name), boost::nowide::widen(checker_path));

	if (cla.should_send.value() && m_single_instance_checker->IsAnotherRunning()) {
#else // mac & linx
	// get_lock() creates the lockfile therefore *cla.should_send is checked after
	if (instance_check_internal::get_lock(lock_name + ".lock", user_data_dir() + "/cache/") && *cla.should_send) {
#endif
		instance_check_internal::send_message(cla.cl_string, lock_name);
		BOOST_LOG_TRIVIAL(error) << "Instance check: Another instance found. This instance will terminate. Lock file of current running instance is located at " << user_data_dir() << 
#ifdef _WIN32
			"\\cache\\"
#else // mac & linx
			"/cache/"
...
GUI::OtherInstanceMessageHandler* other_instance_message_handler()
{
	if (!m_other_instance_message_handler)
		m_other_instance_message_handler = std::make_unique<GUI::OtherInstanceMessageHandler>();
	return m_other_instance_message_handler.get();
}
...
#if defined(__APPLE__)
	this->register_for_messages(AppAdapter::gui_app()->get_instance_hash_string());
#endif //__APPLE__
...
void OtherInstanceMessageHandler::shutdown(wxFrame* main_frame)
{
...
#ifdef _WIN32 
void OtherInstanceMessageHandler::init_windows_properties(HWND main_frame)
{
	size_t       minor_hash = m_instance_hash_int & 0xFFFFFFFF;
	size_t       major_hash = (m_instance_hash_int & 0xFFFFFFFF00000000) >> 32;
	HWND         hwnd = main_frame;
	...
	//BOOST_LOG_TRIVIAL(debug) << "window properties initialized " << m_instance_hash_int << " (" << minor_hash << " & "<< major_hash;
}
...
{
	std::wstring instance_hash = boost::nowide::widen(AppAdapter::gui_app()->get_instance_hash_string());
	TCHAR 		 wndText[1000];
	TCHAR 		 className[1000];
...
void OtherInstanceMessageHandler::handle_message(const std::string& message) 
{
	...
	if (! paths.empty()) {
		//wxEvtHandler* evt_handler = AppAdapter::plater(); //assert here?
		//if (evt_handler) {
...
{
	instance_check_internal::get_lock(AppAdapter::gui_app()->get_instance_hash_string() + ".lock", user_data_dir() + "/cache/");
}
#endif //__APPLE__
...
namespace MessageHandlerDBusInternal
{
	...
	static void handle_method_another_instance(DBusConnection *connection, DBusMessage *request)
	{
	    ...
	    other_instance_message_handler()->handle_message(text);

		evt_handler = AppAdapter::plater();
		if (evt_handler) {
	...
	static DBusHandlerResult handle_dbus_object_message(DBusConnection *connection, DBusMessage *message, void *user_data)
	{
	    ...
	    std::string our_interface  = "com.softfever3d.orca-slicer.InstanceCheck.Object" + AppAdapter::gui_app()->get_instance_hash_string();
...
void OtherInstanceMessageHandler::listen()
{
    ...
    std::string 		 instance_hash  = AppAdapter::gui_app()->get_instance_hash_string();
