...
-(void)message_update:(NSNotification *)msg
{
	[self bring_forward];
	//pass message  
	other_instance_message_handler()->handle_message(std::string([msg.userInfo[@"data"] UTF8String]));
}
...
-(void)closing_update:(NSNotification *)msg
{
	//[self bring_forward];
	//pass message  
	other_instance_message_handler()->handle_message_other_closed();
}
