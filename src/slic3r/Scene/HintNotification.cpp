...
namespace {
	...
	{
		boost::nowide::ofstream file((boost::filesystem::path(user_data_dir()) / "user" / "hints.cereal").string(), std::ios::binary);
	...
	{
		boost::filesystem::path path(boost::filesystem::path(user_data_dir()) / "user" / "hints.cereal");
	...
	TagCheckResult tag_check_mode(const std::string& tag)
	{
		...
		{
			ConfigOptionMode config_mode = (ConfigOptionMode)app_get_mode();
	...
	TagCheckResult tag_check_tech(const std::string& tag)
	{
		...
		if (std::find(allowed_tags.begin(), allowed_tags.end(), tag) != allowed_tags.end()) {
			const PrinterTechnology tech = app_preset_bundle()->printers.get_edited_preset().printer_technology();
			if (tech == ptFFF) {
				// MMU / FFF
				bool is_mmu = preset_extruders_edited_cnt() > 1;
	...
	{
		if (const GUI::Tab* tab = AppAdapter::gui_app()->get_tab(Preset::Type::TYPE_FILAMENT)) {
			// search PrintConfig filament_type to find if allowed tag
			if (AppAdapter::app_config()->get("filament_type").find(tag)) {
		...
		/* TODO: SLA materials
		else if (const GUI::Tab* tab = AppAdapter::gui_app()->get_tab(Preset::Type::TYPE_SLA_MATERIAL)) {
			//if (AppAdapter::app_config()->get("material_type").find(tag)) {
				const Preset& preset = tab->m_presets->get_edited_preset();
	...
	{
		open_browser_with_warning_dialog(url);
	}
} //namespace
		...
		if (boost::starts_with(section.first, "hint:")) {
			...
			if (dict.find("hypertext_type") != dict.end()) {
				...
				if (dict["hypertext_type"] == "link") {
					...
					HintData		hint_data{ id_string, text1, weight, was_displayed, hypertext_text, follow_text, disabled_tags, enabled_tags, true, documentation_link, img_url, [opt, type, category]() { GUI::AppAdapter::gui_app()->sidebar().jump_to_option(opt, type, category); } };
					m_loaded_hints.emplace_back(hint_data);
					// open preferences
				}
					...
					HintData	hint_data{ id_string, text1, weight, was_displayed, hypertext_text, follow_text, disabled_tags, enabled_tags, false, documentation_link, img_url, [page, item]() { AppAdapter::gui_app()->open_preferences(1, page); } };// 1 is to modify
					m_loaded_hints.emplace_back(hint_data);
				}
				else if (dict["hypertext_type"] == "plater") {
					std::string	item = dict["hypertext_plater_item"];
					HintData	hint_data{ id_string, text1, weight, was_displayed, hypertext_text, follow_text, disabled_tags, enabled_tags, true, documentation_link, img_url, [item]() { AppAdapter::plater()->canvas3D()->highlight_toolbar_item(item); } };
					m_loaded_hints.emplace_back(hint_data);
				}
				else if (dict["hypertext_type"] == "gizmo") {
					std::string	item = dict["hypertext_gizmo_item"];
					HintData	hint_data{ id_string, text1, weight, was_displayed, hypertext_text, follow_text, disabled_tags, enabled_tags, true, documentation_link, img_url, [item]() { AppAdapter::plater()->canvas3D()->highlight_gizmo(item); } };
					m_loaded_hints.emplace_back(hint_data);
				}
					...
					HintData	hint_data{ id_string, text1, weight, was_displayed, hypertext_text, follow_text, disabled_tags, enabled_tags, false, documentation_link, img_url, []() {
						...
						AppAdapter::plater()->canvas_facade()->deselect_all();
					} };
					m_loaded_hints.emplace_back(hint_data);
				}
					...
					HintData	hint_data{ id_string, text1, weight, was_displayed, hypertext_text, follow_text, disabled_tags, enabled_tags, true, documentation_link, img_url, [menu, item]() {
						// AppAdapter::main_panel()->open_menubar_item(menu, item); 
						} 
					};
					m_loaded_hints.emplace_back(hint_data);
				}
...
void NotificationManager::HintNotification::render_close_button(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
	...
	if (!m_documentation_link.empty() && AppAdapter::app_config()->get("suppress_hyperlinks") != "1")
	{
...
{
	auto scale = AppAdapter::plater()->get_current_canvas3D()->get_scale();
	ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.0f, .0f, .0f, .0f));
	ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(.0f, .0f, .0f, .0f));
