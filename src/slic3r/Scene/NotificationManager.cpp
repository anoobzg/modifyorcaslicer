...
{
    if (!m_is_dark_inited) {
        m_is_dark        = AppAdapter::plater()->get_current_canvas3D()->get_dark_mode_status();
        m_is_dark_inited = true;
    }

	if (!m_WindowRadius_inited) {
        m_WindowRadius = 4.0f * AppAdapter::plater()->get_current_canvas3D()->get_scale();
...
{ 
	...
	AppAdapter::plater()->get_current_canvas3D()->schedule_extra_frame(0);
}

void NotificationManager::PopNotification::reset_timer() 
{
	m_notification_start = GLCanvas3D::timestamp_now();
	m_state = EState::Shown; 
}
...
void NotificationManager::PlaterWarningNotification::show()            
{
...
void NotificationManager::URLDownloadNotification::set_percentage(float percent)
{
...
void NotificationManager::push_slicing_error_notification(const std::string &text, std::vector<ModelObject const *> objs)
{
	...
	if (!objs.empty()) {
		callback = [ids](wxEvtHandler*) {
			...
			if (!ovs.empty()) {
				AppAdapter::main_panel()->select_tab(MainPanel::tp3DEditor);
				AppAdapter::obj_list()->select_items(ovs);
			}
			return false;
		};
	}
...
void NotificationManager::update_slicing_notif_dailytips(bool need_change)
{
	for (std::unique_ptr<PopNotification>& notification : m_pop_notifications) {
		if (notification->get_type() == NotificationType::SlicingProgress) {
			...
			if (need_change) {
				AppAdapter::plater()->get_dailytips()->close();
	...
	AppAdapter::plater()->init_notification_manager();
}
...
void NotificationManager::progress_indicator_set_progress(int pr)
{
	for (std::unique_ptr<PopNotification>& notification : m_pop_notifications) {
		if (notification->get_type() == NotificationType::ProgressIndicator) {
			...
			AppAdapter::plater()->get_current_canvas3D()->schedule_extra_frame(100);
			return;
		}
	}
	init_progress_indicator();
...
bool NotificationManager::push_notification_data(std::unique_ptr<NotificationManager::PopNotification> notification, int timestamp)
{
	...
	if (!m_initialized)
		return retval;
	GLCanvas3D& canvas = *AppAdapter::plater()->get_current_canvas3D();
                                                         ...
                                                         int64_t                                               delay_interval)
{
	...
	AppAdapter::plater()->get_current_canvas3D()->schedule_extra_frame(initial_delay == 0 ? delay_interval : initial_delay);
}
...
bool NotificationManager::update_notifications(GLCanvas3D& canvas)
{
	...
	wxWindow* p = dynamic_cast<wxWindow*>(AppAdapter::plater());
...
{
	return GUI::AppAdapter::plater()->get_notification_manager();
}
