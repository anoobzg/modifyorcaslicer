...
#include "slic3r/Global/ProjectFlow.hpp"
...
void Downloader::start_download(const std::string& full_url)
{
	...
	main_panel->Freeze();
	main_panel->select_tab((size_t)MainPanel::TabPosition::tp3DEditor);

    plater->select_view3d();
	...
	if (!boost::regex_search(full_url, results, re) && !boost::regex_search(full_url, results, re2)) {
        ...
        // Orca: show error
        NotificationManager* ntf_mngr = get_notification_manager();
    ...
    if (is_bambustudio_open(full_url) || (is_orca_open(full_url) && is_makerworld_link(full_url)))
        request_model_download(wxString::FromUTF8(escaped_url));
    else {
...
void Downloader::on_complete(wxCommandEvent& event)
{
	...
	AppAdapter::plater()->load_files(paths);
}
