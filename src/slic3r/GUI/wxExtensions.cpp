...
wxBitmap* get_default_extruder_color_icon(bool thin_icon/* = false*/)
{
    ...
    const double em = app_em_unit();
    const int icon_width = lround((thin_icon ? 2 : 4.5) * em);
    const int icon_height = lround(2 * em);
    bool dark_mode = Slic3r::GUI::dark_mode();

    wxClientDC cdc((wxWindow*)Slic3r::GUI::AppAdapter::main_panel());
    wxMemoryDC dc(&cdc);
    dc.SetFont(Slic3r::GUI::Font::Body_12);
...
{
    const wxFont& new_font = focus ? app_bold_font() : app_normal_font();
...
{
    SetBackgroundColour(Slic3r::GUI::StaticBox::GetParentBackgroundColor(parent));
...
{
    Slic3r::GUI::UpdateDarkUI(this, m_has_border);
...
BlinkingBitmap::BlinkingBitmap(wxWindow* parent, const std::string& icon_name) :
    wxStaticBitmap(parent, wxID_ANY, wxNullBitmap, wxDefaultPosition, wxSize(int(1.6 * app_em_unit()), -1))
{
