...
{
    wxColour def_colour = get_label_clr_default();//wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT);
...
UnsavedChangesDialog::UnsavedChangesDialog(const wxString &caption, const wxString &header, const std::string &app_config_key, int act_buttons)
    : DPIDialog(static_cast<wxWindow *>(AppAdapter::mainframe()),
                wxID_ANY,
...
inline int UnsavedChangesDialog::ShowModal()
{
    ...
    auto choise     = app_get(choise_key);
    long result = 0;
    ...
    if (r != wxID_CANCEL && dynamic_cast<CheckUIBox*>(FindWindowById(wxID_APPLY))->GetValue()) {
        app_set(choise_key, std::to_string(int(m_exit_action)));
    }
    return r;
}
...
void UnsavedChangesDialog::build(Preset::Type type, PresetCollection *dependent_presets, const std::string &new_selected_preset, const wxString &header)
{
    ...
    m_action_line->SetFont(Font::Body_13);
    ...
    static_temp_title->SetFont(Font::Body_13);
    ...
    static_oldv_title->SetFont(Font::Body_13);
    ...
    static_newv_title->SetFont(Font::Body_13);
   ...
   /* m_info_line = new wxStaticText(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(-1, 44), 0);
     m_info_line->SetFont(Font::Body_13);
     m_info_line->Wrap(-1);
    ...
    auto checkbox       = new CheckUIBox(this, wxID_APPLY);
    ...
    checkbox_text->SetFont(Font::Body_13);
    ...
    if (ActionButtons::TRANSFER & m_buttons) {
        const PresetCollection* switched_presets = type == Preset::TYPE_INVALID ? nullptr : AppAdapter::gui_app()->get_tab(type)->get_presets();
...
void UnsavedChangesDialog::update_list()
{
    ...
    for (auto iter = class_c_list.begin(); iter != class_c_list.end(); iter++) {
        ...
        text_category->SetFont(Font::Head_13);
        ...
        for (auto i = 0; i < iter->second.size(); i++) {
            ...
            for (auto g = 0; g < class_g_list[gname].size(); g++) {
                ...
                if (g == 0) {
                     ...
                     text_left->SetFont(Font::Head_13);
                     text_left->Wrap(-1);
                ...
                text_left->SetFont(Font::Body_13);
                text_left->Wrap(-1);
                text_left->SetForegroundColour(GREY700);
                ...
                text_oldv->SetFont(Font::Body_13);
                text_oldv->Wrap(-1);
                ...
                text_newv->SetFont(Font::Body_13);
                text_newv->Wrap(-1);
...
void UnsavedChangesDialog::update_tree(Preset::Type type, PresetCollection* presets_)
{
    ...
    for (PresetCollection* presets : presets_list)
    {
        ...
        const std::map<wxString, std::string>& category_icon_map = AppAdapter::gui_app()->get_tab(type)->get_category_icon_map();
    ...
    : wxDialog(nullptr, wxID_ANY, option_name, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    UpdateDarkUI(static_cast<wxButton*>(this->FindWindowById(wxID_OK, this)), true);
...
static PresetCollection* get_preset_collection(Preset::Type type, PresetBundle* preset_bundle = nullptr) {
    if (!preset_bundle)
        preset_bundle = app_preset_bundle();
...
void DiffPresetDialog::create_buttons()
{
    ...
    auto enable_transfer = [this](const Preset::Type& type) {
        const Preset& main_edited_preset = get_preset_collection(type, app_preset_bundle())->get_edited_preset();
...
DiffPresetDialog::DiffPresetDialog(wxWindow* parent)
    : DPIDialog(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER),
    m_pr_technology(app_preset_bundle()->printers.get_edited_preset().printer_technology())
{
    ...
    assert(app_preset_bundle());

    m_preset_bundle_left  = std::make_unique<PresetBundle>(*app_preset_bundle());
    m_preset_bundle_right = std::make_unique<PresetBundle>(*app_preset_bundle());
...
{
    *m_preset_bundle_left  = *app_preset_bundle();
    *m_preset_bundle_right = *app_preset_bundle();
...
{
#ifdef _WIN32
    UpdateAllStaticTextDarkUI(this);
    UpdateDarkUI(m_show_all_presets);
    UpdateDVCDarkUI(m_tree);
#endif
