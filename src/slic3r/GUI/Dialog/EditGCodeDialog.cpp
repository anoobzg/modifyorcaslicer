...
//        this->on_search_update();
    ...
    m_params_list->SetFont(app_code_font());
    UpdateDarkUI(m_params_list);
    ...
    m_gcode_editor->SetFont(app_code_font());
    m_gcode_editor->SetInsertionPointEnd();
    UpdateDarkUI(m_gcode_editor);
    ...
    m_param_label->SetFont(app_bold_font());
...
wxDataViewItem EditGCodeDialog::add_presets_placeholders()
{
    ...
    const bool  is_fff      = AppAdapter::plater()->printer_technology() == ptFFF;
    ...
    const auto& tab_list    = AppAdapter::gui_app()->tabs_list;
...
void ParamsNode::RefreshSearch(const wxString& search_text)
{
    ...
    if (GetEnabledChildren().empty())
        if (auto pos = text.find(search_text); IsParamNode() && pos != wxString::npos) {
            m_highlight_index = std::make_unique<std::pair<int, int>>(pos, search_text.Len());
            Enable();
        } else {
            Disable();
        }
