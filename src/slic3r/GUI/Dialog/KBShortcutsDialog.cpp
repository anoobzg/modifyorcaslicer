...
KBShortcutsDialog::KBShortcutsDialog(wxWindow* parent)
    : DPIDialog(parent, wxID_ANY,_L("Keyboard Shortcuts"),
    wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE)
{
    // fonts
    const wxFont& font = app_normal_font();
    const wxFont& bold_font = app_bold_font();
    SetFont(font);
...
void KBShortcutsDialog::OnSelectTabel(wxCommandEvent &event)
{
    ...
    while (i != m_hash_selector.end()) {
        ...
        if (id == sel->m_index) {
            ...
            sel->m_tab_text->SetFont(Font::Head_13);
            ...
            sel->m_tab_text->SetFont(Font::Body_13);
            sel->m_tab_button->Refresh();
            sel->m_tab_text->Refresh();
        }
        i++;
