...
std::string get_main_info(bool format_as_html)
{
    ...
    if (!format_as_html)
        out << b_start << (SLIC3R_APP_FULL_NAME) << b_end << line_end;
...
std::string get_mem_info(bool format_as_html)
{
    ...
    const Slic3r::UndoRedo::Stack &stack = AppAdapter::plater()->undo_redo_stack_main();
...
SysInfoDialog::SysInfoDialog()
    : DPIDialog(static_cast<wxWindow*>(AppAdapter::main_panel()), wxID_ANY, wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("System Information"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
	wxColour bgr_clr = AppAdapter::gui_app()->get_window_default_clr();//wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOW);
	SetBackgroundColour(bgr_clr);
    SetFont(app_normal_font());

    wxBoxSizer* hsizer = new wxBoxSizer(wxHORIZONTAL);
    hsizer->SetMinSize(wxSize(50 * app_em_unit(), -1));

	auto main_sizer = new wxBoxSizer(wxVERTICAL);
	main_sizer->Add(hsizer, 1, wxEXPAND | wxALL, 10);

    // logo
    m_logo_bmp = ScalableBitmap(this, app_logo_name(), 192);
    ...
    // title
    {
        wxStaticText* title = new wxStaticText(this, wxID_ANY, SLIC3R_APP_FULL_NAME, wxDefaultPosition, wxDefaultSize);
        wxFont title_font = app_bold_font();
        title_font.SetFamily(wxFONTFAMILY_ROMAN);
        title_font.SetPointSize(22);
        title->SetFont(title_font);
        vsizer->Add(title, 0, wxEXPAND | wxALIGN_LEFT | wxTOP, app_em_unit()/*50*/);
    }

    // main_info_text
    wxFont font = get_default_font(this);
    const auto text_clr = AppAdapter::gui_app()->get_label_clr_default();//wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT);
    ...
    m_html = new wxHtmlWindow(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHW_SCROLLBAR_NEVER);
    {
		...
		const auto text = wxString::Format(
            ...
            get_main_info(true));
        m_html->SetPage(text);
        vsizer->Add(m_html, 1, wxEXPAND | wxBOTTOM, app_em_unit());
    }

    // opengl_info
    m_opengl_info_html = new wxHtmlWindow(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHW_SCROLLBAR_AUTO);
    {
        m_opengl_info_html->SetMinSize(wxSize(-1, 16 * app_em_unit()));
       ...
       const auto text = GUI::format_wxstr(
            ...
            get_mem_info(true), AppAdapter::gui_app()->get_gl_info(false),
...
{
    wxTheClipboard->Open();
    const auto text = get_main_info(false) + "\n" + AppAdapter::gui_app()->get_gl_info(true);
