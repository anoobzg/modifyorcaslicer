...
SavePresetDialog::Item::Item(Preset::Type type, const std::string &suffix, wxBoxSizer *sizer, SavePresetDialog *parent) : m_type(type), m_parent(parent)
{
    ...
    label_top->SetFont(Font::Body_13);
    ...
    //
    //    m_combo = new wxComboBox(m_parent, wxID_ANY, from_u8(preset_name), wxDefaultPosition, wxSize(35 * app_em_unit(), -1));
    ...
    m_left_text->Wrap(-1);
    m_left_text->SetFont(Font::Body_13);
    ...
    m_right_text->SetFont(Font::Body_13);
    m_right_text->Wrap(-1);
...
{
    PhysicalPrinterCollection &printers = app_preset_bundle()->physical_printers;
    ...
    action_stb->SetFont(app_bold_font());
...
{
    bool show = app_preset_bundle()->physical_printers.has_selection() && m_old_preset_name != preset_name;
