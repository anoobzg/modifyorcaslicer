...
{
    const AppConfig *app_config = AppAdapter::app_config();
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    excutable_path = escape_path_string_xdg(excutable_path);
    ...
    get_notification_manager()->push_notification(NotificationType::DesktopIntegrationSuccess);
}
void DesktopIntegrationDialog::undo_desktop_intgration()
{
    ...
    get_notification_manager()->push_notification(NotificationType::UndoDesktopIntegrationSuccess);
}
