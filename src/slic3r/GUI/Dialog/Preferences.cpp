...
#include "slic3r/Theme/ThemeSizeDef.hpp"
#include "slic3r/Theme/ThemeColorDef.hpp"
...
#include "slic3r/GUI/Dialog/DialogDef.hpp"
...
std::tuple<wxBoxSizer*, ComboBox*> PreferencesDialog::create_item_combobox_base(wxString title, wxWindow* parent, wxString tooltip, std::string param, std::vector<wxString> vlist, unsigned int current_index)
{
    ...
    combo_title->SetFont(Font::Body_13);
    ...
    combobox->SetFont(Font::Body_13);
    combobox->GetDropDown().SetFont(Font::Body_13);
    ...
    combobox->Bind(wxEVT_COMBOBOX, [this, param, vlist, combobox](wxCommandEvent &e) {
        ...
        if (e.GetString().mb_str() != app_config->get(param)) {
            ...
            auto check = [this](bool yes_or_no) {
                ...
                return AppAdapter::gui_app()->check_and_keep_current_preset_changes(_L("Switching application language"),
            ...
            if (m_current_language_selected >= 0 && m_current_language_selected < vlist.size()) {
                ...
                app_load_language(vlist[m_current_language_selected]->CanonicalName, false);
                Close();
                // Reparent(nullptr);
                GetParent()->RemoveChild(this);
                AppAdapter::gui_app()->recreate_GUI(_L("Changing application language"));
            }
        }
...
wxBoxSizer *PreferencesDialog::create_item_region_combobox(wxString title, wxWindow *parent, wxString tooltip, std::vector<wxString> vlist)
{
    ...
    if (!app_config->get("region").empty()) {
    ...
    combobox->GetDropDown().Bind(wxEVT_COMBOBOX, [this, combobox, current_region, local_regions](wxCommandEvent &e) {
        ...
        app_config->set("region", region.ToStdString());
    ...
    wxString title, wxWindow *parent, wxString tooltip, int padding_left, std::string param, std::vector<wxString> vlista, std::vector<wxString> vlistb)
{
   ...
   combobox_left->SetFont(Font::Body_13);
   combobox_left->GetDropDown().SetFont(Font::Body_13);
   ...
   combobox_right->SetFont(Font::Body_13);
   combobox_right->GetDropDown().SetFont(Font::Body_13);
...
wxBoxSizer *PreferencesDialog::create_item_input(wxString title, wxString title2, wxWindow *parent, wxString tooltip, std::string param, std::function<void(wxString)> onchange)
{
    ...
    input_title->SetFont(Font::Body_13);
    ...
    second_title->SetFont(Font::Body_13);
...
wxBoxSizer *PreferencesDialog::create_item_switch(wxString title, wxWindow *parent, wxString tooltip ,std::string param)
{
    ...
    switch_title->SetFont(Font::Body_13);
...
wxBoxSizer* PreferencesDialog::create_item_darkmode_checkbox(wxString title, wxWindow* parent, wxString tooltip, int padding_left, std::string param)
{
    ...
    checkbox->Bind(wxEVT_TOGGLEBUTTON, [this, checkbox, param](wxCommandEvent& e) {
        ...
        //dark mode
#ifdef _MSW_DARK_MODE
        force_colors_update(AppAdapter::main_panel());
...
void PreferencesDialog::set_dark_mode()
{
    ...
    SetActiveWindow(AppAdapter::main_panel()->GetHWND());
    ...
    checkbox->Bind(wxEVT_TOGGLEBUTTON, [this, checkbox, param](wxCommandEvent &e) {
        ...
        if (param == "developer_mode") {
            ...
            if (m_developer_mode_def == "true") {
                Slic3r::GUI::AppAdapter::gui_app()->save_mode(comDevelop);
            } else {
    ...
    wxString title, wxString title2, wxWindow* parent, wxString tooltip, wxString tooltip2, std::function<void()> onclick, bool button_on_left/* = false*/)
{
    ...
    m_staticTextPath->SetFont(Font::Body_13);
    ...
    m_button_download->SetFont(Font::Body_10);
...
wxBoxSizer* PreferencesDialog::create_item_link_association(wxWindow* parent, wxString url_prefix, wxString website_name)
{
    ...
    bool reg_to_current_instance = check_url_association(url_prefix.ToStdWstring(), registered_bin);
    ...
    registered_instance_title->SetFont(Font::Body_13);
    ...
    auto update_current_association_str = [=, &reg_to_current_instance](){
        ...
        reg_to_current_instance = check_url_association(url_prefix.wc_str(), registered_bin);
    ...
    v_sizer->Add(registered_instance_title, 0, wxLEFT, 60);

    checkbox->Bind(wxEVT_TOGGLEBUTTON, [=](wxCommandEvent& e) {
        associate_url(url_prefix.ToStdWstring());
        checkbox->Disable();
        update_current_association_str();
        e.Skip();
    });
...
wxWindow* PreferencesDialog::create_general_page()
{
    ...
    auto item_save_presets = create_item_button(_L("Clear my choice on the unsaved presets."), _L("Clear"), page, L"", _L("Clear my choice on the unsaved presets."), [this]() {
        app_config->set("save_preset_choise", "");
    });
    ...
    auto item_max_recent_count = create_item_input(_L("Maximum recent projects"), "", page, _L("Maximum count of recent projects"), "max_recent_count", [](wxString value) {
        long max = 0;
        if (value.ToLong(&max))
            AppAdapter::main_panel()->set_max_recent_count(max);
    });
    auto item_save_choise = create_item_button(_L("Clear my choice on the unsaved projects."), _L("Clear"), page, L"", _L("Clear my choice on the unsaved projects."), [this]() {
...
wxWindow* PreferencesDialog::create_debug_page()
{
    ...
    on_select_radio(m_iot_environment_def);
    ...
    debug_button->SetFont(Font::Body_13);
    ...
    debug_button->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent &e) {
        ...
        case wxID_YES: {
            ...
            if (true) {
                app_config->set_iot_environment(param.ToStdString());
