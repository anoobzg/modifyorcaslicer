...
// CopyrightsDialog
// -----------------------------------------
CopyrightsDialog::CopyrightsDialog(wxWindow* parent)
    : DPIDialog(parent, wxID_ANY, from_u8((boost::format("%1% - %2%")
...
{
    wxColour bgr_clr = get_window_default_clr();//wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOW);

    const auto text_clr = get_label_clr_default();// wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT);
...
{
    open_browser_with_warning_dialog(event.GetLinkInfo().GetHref());
    event.Skip(false);
}
...
AboutDialog::AboutDialog(wxWindow* parent)
    : DPIDialog(parent, wxID_ANY,from_u8((boost::format(_utf8(L("About %s"))) % (SLIC3R_APP_FULL_NAME)).str()),wxDefaultPosition,
        wxDefaultSize, /*wxCAPTION*/wxDEFAULT_DIALOG_STYLE)
{
    ...
    // version
    {
        ...
        auto _build_string_font = Font::Body_12;
        // _build_string_font.SetStyle(wxFONTSTYLE_ITALIC);

        vesizer->Add(0, 0, 1, wxEXPAND, FromDIP(5));
        auto          version_string = std::string(LightMaker_VERSION); // _L("Orca Slicer ") + " " + std::string(LightMaker_VERSION);
...
// #endif
    ...
    bool is_zh = app_get("language") == "zh_CN";
    for (int i = 0; i < text_list.size(); i++)
    {
        ...
        staticText->SetFont(Font::Body_12);
        if (is_zh) {
            wxString find_txt = "";
            wxString count_txt = "";
    ...
    button_portions->SetFont(Font::Body_12);
...
{
    wxTheClipboard->Open();
    wxTheClipboard->SetData(new wxTextDataObject(_L("Version") + " " + format_display_version()));
