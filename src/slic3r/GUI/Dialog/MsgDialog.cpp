...
MsgDialog::MsgDialog(wxWindow *parent, const wxString &title, const wxString &headline, long style, wxBitmap bitmap)
	: DPIDialog(parent ? parent : dynamic_cast<wxWindow*>(AppAdapter::main_panel()), wxID_ANY, title, wxDefaultPosition, wxSize(360, -1),wxDEFAULT_DIALOG_STYLE)
	, boldfont(app_normal_font())
	...
	, btn_sizer(new wxBoxSizer(wxHORIZONTAL))
{
 ...
 //   headtext->Wrap(CONTENT_WIDTH*app_em_unit());
...
{
    m_checkbox_dsa = new CheckUIBox(this);
    ...
    m_text_dsa->SetFont(Font::Body_13);
...
void MsgDialog::apply_style(long style)
{
    ...
    logo->SetBitmap( create_scaled_bitmap(style & wxAPPLY        ? "completed" :
                                          style & wxICON_WARNING        ? "exclamation" : // "exclamation" used for dialogs "obj_warning" used for 16x16 areas
                                          style & wxICON_INFORMATION    ? "info"        :
                                          style & wxICON_QUESTION       ? "question"    : "main", this, 64, style & wxICON_ERROR));
}
...
static void add_msg_content(wxWindow* parent, wxBoxSizer* content_sizer, wxString msg, bool monospaced_font = false, bool is_marked_msg = false)
{
    ...
    wxFont      monospace = app_code_font();
    wxColour    text_clr = get_label_clr_default();
	...
	, msg(msg)
{
    ...
    SetMaxSize(wxSize(-1, CONTENT_MAX_HEIGHT*app_em_unit()));
    ...
    : DPIDialog(parent ? parent : nullptr, wxID_ANY, title, wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    m_del_btn->SetFont(Font::Body_12);
...
wxBoxSizer *Newer3mfVersionDialog::get_btn_sizer()
{
    ...
    if (!file_version_newer) {
        ...
        m_update_btn->SetFont(Font::Body_12);
        ...
        m_update_btn->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent &e) {
            EndModal(wxID_OK);
            if (app_has("cloud_software_url")) {
                std::string download_url = app_get("cloud_software_url");
    ...
    m_later_btn->SetFont(Font::Body_12);
