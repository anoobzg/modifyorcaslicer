...
#include "slic3r/Config/Field.hpp"
#include "slic3r/GUI/Dialog/DesktopIntegrationDialog.hpp"
    ...
    , width(0)
{
    ...
    for (const auto &model : models) {
        ...
        if (!load_bitmap(GUI::from_u8(Slic3r::user_data_dir() + "/vendor/" + vendor.id + "/" + model.id + "_thumbnail.png"), bitmap, bitmap_width)) {
    ...
    if (titles.size() > 1 || is_variants) {
        ...
        UpdateDarkUI(sel_all_std);
        UpdateDarkUI(sel_all);
        UpdateDarkUI(sel_none);
    ...
    , list_profile(new PresetList(this))
{
    ...
    UpdateDarkUI(list_printer);
    UpdateDarkUI(list_type);
    UpdateDarkUI(list_vendor);
    UpdateDarkUI(sel_all);
    UpdateDarkUI(sel_none);
    UpdateDarkUI(list_profile);
...
//#endif
//#endif
    const auto text_clr = get_label_clr_default();
...
public:
    DiamTextCtrl(wxWindow* parent)
    {
        ...
        long style = 0;
#endif
        Create(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(Field::def_width_thinner() * app_em_unit(), wxDefaultCoord), style);
        UpdateDarkUI(this);
    }
    ~DiamTextCtrl() {}
};
...
void ConfigWizardIndex::on_paint(wxPaintEvent & evt)
{
    ...
    for (size_t i = 0; i < items.size(); i++) {
        ...
        dc.SetTextForeground(get_label_clr_default());
...
void ConfigWizard::priv::load_vendors()
{
    ...
    if (! app_config->legacy_datadir()) {
        ...
        //BBS: change directories by design
        const auto printer_dir = fs::path(Slic3r::user_data_dir()) / PRESET_SYSTEM_DIR / PRESET_PRINTER_NAME;
...
bool ConfigWizard::priv::apply_config(AppConfig *app_config, PresetBundle *preset_bundle, const PresetUpdater *updater, bool& apply_keeped_changes)
{
    ...
    bool suppress_sla_printer = model_has_multi_part_objects(AppAdapter::gui_app()->model());
    ...
    if (!check_unsaved_preset_changes) {
        if ((check_unsaved_preset_changes = !preferred_model.empty())) {
            ...
            if (!AppAdapter::gui_app()->check_and_keep_current_preset_changes(caption, header, act_btns, &apply_keeped_changes))
                return false;
        }
    ...
    if (!only_sla_mode && page_custom->custom_wanted()) {
        // if unsaved changes was not cheched till this moment
        if (!check_unsaved_preset_changes && 
            !AppAdapter::gui_app()->check_and_keep_current_preset_changes(caption, _L("Custom printer was installed and it will be activated."), act_btns, &apply_keeped_changes))
            return false;
        ...
        custom_config->set_key_value("filament_colour", app_preset_bundle()->project_config.option("filament_colour"));
        ...
        AppAdapter::plater()->sidebar().update_presets(Slic3r::Preset::Type::TYPE_PRINTER);
        AppAdapter::plater()->sidebar().update_presets(Slic3r::Preset::Type::TYPE_FILAMENT);
        AppAdapter::plater()->sidebar().update_presets(Slic3r::Preset::Type::TYPE_PRINT);
    }
    ...
    //wxQueueEvent(AppAdapter::main_panel(), evt);
    ...
    , p(new priv(this))
{
    ...
    UpdateDarkUI(p->btn_prev);
    UpdateDarkUI(p->btn_next);
    UpdateDarkUI(p->btn_finish);
    UpdateDarkUI(p->btn_cancel);
