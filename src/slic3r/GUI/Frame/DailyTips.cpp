...
void DailyTipsDataRenderer::open_wiki() const
{
    ...
    {
        open_browser_with_warning_dialog(m_data.wiki_url);
    }
}
...
void DailyTipsDataRenderer::render_text(const ImVec2& start_pos, const ImVec2& size) const
{
    ...
    if (!is_zh) {
        ...
        Label* wrapped_text = new Label(AppAdapter::app()->GetTopWindow());
        wrapped_text->Hide();
    ...
    m_dailytips_renderer(std::make_unique<DailyTipsDataRenderer>(layout))
{
    ...
    m_is_expanded = AppAdapter::app_config()->get("show_hints") == "true";
}
...
void DailyTipsPanel::expand(bool expand)
{
    ...
    AppAdapter::app_config()->set_bool("show_hints", expand);
}
...
void DailyTipsPanel::collapse()
{
    if (!m_can_expand)
        return;
    m_is_expanded = false;
    AppAdapter::app_config()->set_bool("show_hints", false);
}
