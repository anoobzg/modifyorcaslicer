...
CheckUIBox::CheckUIBox(wxWindow *parent, int id)
    ...
    , m_off_focused(this, "check_off_focused", 18)
{
    ...
    Bind(wxEVT_SET_FOCUS, &CheckUIBox::updateBitmap, this);
    Bind(wxEVT_KILL_FOCUS, &CheckUIBox::updateBitmap, this);
    Bind(wxEVT_ENTER_WINDOW, &CheckUIBox::updateBitmap, this);
    Bind(wxEVT_LEAVE_WINDOW, &CheckUIBox::updateBitmap, this);
#endif
...
void CheckUIBox::SetValue(bool value)
{
	wxBitmapToggleButton::SetValue(value);
	update();
}

void CheckUIBox::SetHalfChecked(bool value)
{
	m_half_checked = value;
	update();
}

void CheckUIBox::Rescale()
{
    m_on.msw_rescale();
    m_half.msw_rescale();
    m_off.msw_rescale();
...
CheckUIBox::State CheckUIBox::GetNormalState() const { return State_Normal; }
...
bool CheckUIBox::Enable(bool enable)
{
...
wxBitmap CheckUIBox::DoGetBitmap(State which) const
{
    if (m_disable) {
...
void CheckUIBox::updateBitmap(wxEvent & evt)
{
    evt.Skip();
