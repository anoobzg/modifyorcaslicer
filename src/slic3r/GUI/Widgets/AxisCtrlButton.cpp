...
void AxisCtrlButton::render(wxDC& dc)
{
	...
	//draw text
    if (!IsEnabled())
        gc->SetFont(Font::Body_12, text_color.colorForStates(StateColor::Disabled));
    else
	    gc->SetFont(Font::Head_12, text_color.colorForStates(states));
	wxDouble w, h;
	...
	gc->SetFont(Font::Body_12, text_num_color);
