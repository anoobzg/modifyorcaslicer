...
{
    SetFont(Slic3r::GUI::app_normal_font());
#ifdef _WIN32
...
int BitmapComboBox::Append(const wxString& item)
{
    ...
    wxBitmap bitmap(1, int(1.6 * app_em_unit() + 1));
    {
...
void BitmapComboBox::DrawBackground_(wxDC& dc, const wxRect& rect, int WXUNUSED(item), int flags) const
{
    if (flags & wxODCB_PAINTING_SELECTED)
    {
        ...
        dc.SetTextForeground(get_label_highlight_clr());

        wxColour selCol = get_highlight_default_clr();
        dc.SetPen(selCol);
        dc.SetBrush(selCol);
    ...
    {
        dc.SetTextForeground(flags & ODCB_PAINTING_DISABLED ? wxColour(108,108,108) : get_label_clr_default());

        wxColour selCol = flags & ODCB_PAINTING_DISABLED ? 
            get_highlight_default_clr() :
            get_window_default_clr();
        dc.SetPen(selCol);
        dc.SetBrush(selCol);
