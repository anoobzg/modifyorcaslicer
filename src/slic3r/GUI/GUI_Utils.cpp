...
#else
    const unsigned luma = AppAdapter::gui_app()->get_colour_approx_luma(wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOW));
    return luma < 128;
#endif
}
...
void update_dark_ui(wxWindow* window)
{
    ...
    bool is_dark = AppAdapter::app_config()->get("dark_color_mode") == "1";
#else
    bool is_dark = false;
#endif
