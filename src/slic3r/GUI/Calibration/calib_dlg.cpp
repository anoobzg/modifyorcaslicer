...
wxBoxSizer* create_item_checkbox(wxString title, wxWindow* parent, bool* value, CheckUIBox*& checkbox)
{
    wxBoxSizer* m_sizer_checkbox = new wxBoxSizer(wxHORIZONTAL);

    m_sizer_checkbox->Add(0, 0, 0, wxEXPAND | wxLEFT, 5);

    checkbox = new CheckUIBox(parent);
...
MultiNozzle_Calibration_Dlg::MultiNozzle_Calibration_Dlg(wxWindow* parent, wxWindowID id, Plater* plater)
    : DPIDialog(parent, id, _L("Multi-Nozzle Calibration"), wxDefaultPosition, parent->FromDIP(wxSize(-1, 320)), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER), m_plater(plater)
{
    wxBoxSizer* v_sizer = new wxBoxSizer(wxVERTICAL);
    SetSizer(v_sizer);

    // Settings
    wxString length_str = _L("Rectangle Length (mm): ");
    wxString width_str = _L("Rectangle Width (mm): ");
    wxString height_str = _L("Print Height (mm): ");
    wxString spacing_str = _L("Spacing between rectangles (mm): ");
    
    auto text_size = wxWindow::GetTextExtent(length_str);
    text_size.IncTo(wxWindow::GetTextExtent(width_str));
    text_size.IncTo(wxWindow::GetTextExtent(height_str));
    text_size.IncTo(wxWindow::GetTextExtent(spacing_str));
    text_size.x = text_size.x * 1.2;
    
    wxStaticBoxSizer* settings_sizer = new wxStaticBoxSizer(wxVERTICAL, this, _L("Settings"));

    wxString input_text_str = _L("mm");
    auto input_text_size = wxWindow::GetTextExtent(input_text_str);
    ...
    auto length_text = new wxStaticText(this, wxID_ANY, length_str, wxDefaultPosition, st_size, wxALIGN_LEFT);
    m_tiLength = new TextInput(this, std::to_string(10), _L("mm"), "", wxDefaultPosition, ti_size, wxTE_CENTRE);
    m_tiLength->GetTextCtrl()->SetValidator(wxTextValidator(wxFILTER_NUMERIC));
    length_sizer->Add(length_text, 0, wxALL | wxALIGN_CENTER_VERTICAL, 2);
    length_sizer->Add(m_tiLength, 0, wxALL | wxALIGN_CENTER_VERTICAL, 2);
    settings_sizer->Add(length_sizer);

    // Rectangle width
    auto width_sizer = new wxBoxSizer(wxHORIZONTAL);
    auto width_text = new wxStaticText(this, wxID_ANY, width_str, wxDefaultPosition, st_size, wxALIGN_LEFT);
    m_tiWidth = new TextInput(this, std::to_string(2), _L("mm"), "", wxDefaultPosition, ti_size, wxTE_CENTRE);
    m_tiWidth->GetTextCtrl()->SetValidator(wxTextValidator(wxFILTER_NUMERIC));
    width_sizer->Add(width_text, 0, wxALL | wxALIGN_CENTER_VERTICAL, 2);
    width_sizer->Add(m_tiWidth, 0, wxALL | wxALIGN_CENTER_VERTICAL, 2);
    settings_sizer->Add(width_sizer);

    // Print height
    auto print_height_sizer = new wxBoxSizer(wxHORIZONTAL);
    auto print_height_text = new wxStaticText(this, wxID_ANY, height_str, wxDefaultPosition, st_size, wxALIGN_LEFT);
    m_tiPrintHeight = new TextInput(this, std::to_string(10), _L("mm"), "", wxDefaultPosition, ti_size, wxTE_CENTRE);
    m_tiPrintHeight->GetTextCtrl()->SetValidator(wxTextValidator(wxFILTER_NUMERIC));
    print_height_sizer->Add(print_height_text, 0, wxALL | wxALIGN_CENTER_VERTICAL, 2);
    print_height_sizer->Add(m_tiPrintHeight, 0, wxALL | wxALIGN_CENTER_VERTICAL, 2);
    settings_sizer->Add(print_height_sizer);

    // Spacing between rectangles
    auto spacing_sizer = new wxBoxSizer(wxHORIZONTAL);
    auto spacing_text = new wxStaticText(this, wxID_ANY, spacing_str, wxDefaultPosition, st_size, wxALIGN_LEFT);
    m_tiSpacing = new TextInput(this, std::to_string(20), _L("mm"), "", wxDefaultPosition, ti_size, wxTE_CENTRE);
    m_tiSpacing->GetTextCtrl()->SetValidator(wxTextValidator(wxFILTER_NUMERIC));
    spacing_sizer->Add(spacing_text, 0, wxALL | wxALIGN_CENTER_VERTICAL, 2);
    spacing_sizer->Add(m_tiSpacing, 0, wxALL | wxALIGN_CENTER_VERTICAL, 2);
    settings_sizer->Add(spacing_sizer);
    ...
    m_btnStart->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &MultiNozzle_Calibration_Dlg::on_start, this);

    auto btn_sizer = new wxBoxSizer(wxHORIZONTAL);
    btn_sizer->AddStretchSpacer();
    btn_sizer->Add(m_btnStart, 0, wxALL, FromDIP(5));
    btn_sizer->AddStretchSpacer();
    v_sizer->Add(btn_sizer, 0, wxEXPAND);

    Layout();
    Fit();
}

MultiNozzle_Calibration_Dlg::~MultiNozzle_Calibration_Dlg() {
    m_btnStart->Disconnect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(MultiNozzle_Calibration_Dlg::on_start), NULL, this);
}

void MultiNozzle_Calibration_Dlg::on_start(wxCommandEvent& event) {
    double length, width, print_height, spacing;
    
    bool read_double = m_tiLength->GetTextCtrl()->GetValue().ToDouble(&length);
    read_double = read_double && m_tiWidth->GetTextCtrl()->GetValue().ToDouble(&width);
    read_double = read_double && m_tiPrintHeight->GetTextCtrl()->GetValue().ToDouble(&print_height);
    read_double = read_double && m_tiSpacing->GetTextCtrl()->GetValue().ToDouble(&spacing);

    if (!read_double || length <= 0 || width <= 0 || print_height <= 0 || spacing < 0) {
        MessageDialog msg_dlg(nullptr, _L("Please input valid values:\nLength > 0\nWidth > 0\nPrint Height > 0\nSpacing >= 0"), 
    ...
    m_params.mode = CalibMode::Calib_Multi_Nozzle;
    m_params.start = length;
    m_params.end = width;
    m_params.step = spacing;
    m_params.print_numbers = false;
    m_params.accelerations.clear();
    m_params.accelerations.push_back(print_height);
    
    m_plater->calib_multi_nozzle(m_params);
    EndModal(wxID_OK);
}

void MultiNozzle_Calibration_Dlg::on_dpi_changed(const wxRect& suggested_rect) {
    this->Refresh();
    Fit();
}
