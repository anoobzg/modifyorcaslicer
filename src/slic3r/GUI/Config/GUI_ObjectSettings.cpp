...
#include "slic3r/Config/ConfigManipulation.hpp"
...
{
    if (!AppAdapter::mainframe()->IsShown())
        return false;

    auto objects_ctrl   = AppAdapter::obj_list();
    auto objects_model  = AppAdapter::obj_list()->GetModel();
    ...
    for (auto item : items) {
        ...
        assert(obj_idx >= 0);
        auto object = AppAdapter::gui_app()->model().objects[obj_idx];
        if (type == itObject) {
    ...
    auto tab_plate = dynamic_cast<TabPrintPlate*>(AppAdapter::gui_app()->get_plate_tab());
    auto tab_object = dynamic_cast<TabPrintModel*>(AppAdapter::gui_app()->get_model_tab());
    auto tab_volume = dynamic_cast<TabPrintModel*>(AppAdapter::gui_app()->get_model_tab(true));
    auto tab_layer = dynamic_cast<TabPrintModel*>(AppAdapter::gui_app()->get_layer_tab());
...
{
    const DynamicPrintConfig& print_config = AppAdapter::plater()->printer_technology() == ptFFF ?
...
{
    const auto objects_model        = AppAdapter::obj_list()->GetModel();
    const auto item                 = AppAdapter::obj_list()->GetSelection();
    const auto printer_technology   = AppAdapter::plater()->printer_technology();
...
//            });
    ...
    if (!is_object_settings)
    {
        ...
        main_config.apply(AppAdapter::gui_app()->model().objects[obj_idx]->config.get(), true);
