...
#include "slic3r/GUI/Dialog/SingleChoiceDialog.hpp"
...
#include "slic3r/Scene/ExtraRenderers.hpp"
...
{
    return app_preset_bundle()->printers.get_selected_preset().printer_technology();
}

static const Selection& scene_selection()
{
    return AppAdapter::plater()->get_selection();
}

// Config from current edited printer preset
static DynamicPrintConfig& printer_config()
{
    return app_preset_bundle()->printers.get_edited_preset().config;
}

static int filaments_count()
{
    return preset_filaments_cnt();
}
    ...
    wxDataViewCtrl(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxDV_MULTIPLE)
{
    ...
    SetFont(sysFont(13));
#ifdef __WXMSW__
    GenericGetHeader()->SetFont(sysFont(13));
    ...
    Bind(wxEVT_DATAVIEW_SELECTION_CHANGED, [this](wxDataViewEvent& event) {
        ...
        if (sel_node && (sel_node->GetType() & ItemType::itPlate)) {
            if (AppAdapter::plater()->is_preview_shown()) {
                AppAdapter::plater()->select_sliced_plate(sel_node->GetPlateIdx());
            } else {
                AppAdapter::plater()->select_plate(sel_node->GetPlateIdx());
            }
            AppAdapter::plater()->deselect_all();
        }
        else {
            selection_changed();
        }
...
void ObjectList::set_min_height()
{
    ...
    if (m_items_count == size_t(-1))
        m_items_count = 7;
    int list_min_height = lround(2.25 * (m_items_count + 1) * app_em_unit()); // +1 is for height of control header
...
void ObjectList::set_tooltip_for_item(const wxPoint& pt)
{
    ...
    else if (col->GetModelColumn() == (unsigned int)colName && (pt.x >= 2 * app_em_unit() && pt.x <= 4 * app_em_unit()))
    {
...
#endif
    PartPlateList& list = AppAdapter::plater()->get_partplate_list();
    for (size_t i = 0; i < m_objects->size(); ++i)
    {
        wxDataViewItem item = m_objects_model->GetItemById(i);
        if (!item) continue;
...
void ObjectList::object_config_options_changed(const ObjectVolumeID& ov_id)
{
    ...
    ModelObjectPtrs& objects = AppAdapter::gui_app()->model().objects;
...
void ObjectList::printable_state_changed(const std::vector<ObjectVolumeID>& ov_ids)
{
    ...
    AppAdapter::plater()->get_view3D_canvas3D()->update_instance_printable_state_for_objects(obj_idxs);
...
void ObjectList::update_name_in_model(const wxDataViewItem& item) const
{
    if (m_objects_model->GetItemType(item) & itPlate) {
        ...
        if (plate_idx >= 0) {
            auto plate = AppAdapter::plater()->get_partplate_list().get_plate(plate_idx);
...
void ObjectList::selection_changed()
{
    ...
    if (const wxDataViewItem item = GetSelection())
    {
        ...
        if (type & (itLayer|itLayerRoot)) {
            AppAdapter::gui_app()->obj_layers()->reset_selection();

            if (type & itLayerRoot)
                AppAdapter::plater()->canvas3D()->handle_sidebar_focus_event("", false);
            else {
                AppAdapter::gui_app()->obj_layers()->set_selectable_range(m_objects_model->GetLayerRangeByItem(item));
                AppAdapter::gui_app()->obj_layers()->update_scene_from_editor_selection();
            }
        }
    }
...
void ObjectList::paste_objects_into_list(const std::vector<size_t>& object_idxs)
{
    ...
    AppAdapter::plater()->changed_objects(object_idxs);
...
void ObjectList::OnContextMenu(wxDataViewEvent& evt)
{
    ...
    GLCanvas3D* canvas = AppAdapter::plater()->canvas3D();
...
void ObjectList::list_manipulation(const wxPoint& mouse_pos, bool evt_context_menu/* = false*/)
{
    ...
    if (col != nullptr)
    {
	    ...
	    if (col_num == colPrint)
            ...
            get_selected_item_indexes(obj_idx, vol_idx, item);
            //AppAdapter::plater()->PopupObjectTable(obj_idx, vol_idx, mouse_pos);
            if (m_objects_model->GetItemType(item) & itPlate)
                dynamic_cast<TabPrintPlate*>(AppAdapter::gui_app()->get_plate_tab())->reset_model_config();
            else if (m_objects_model->GetItemType(item) & itLayer)
                dynamic_cast<TabPrintLayer*>(AppAdapter::gui_app()->get_layer_tab())->reset_model_config();
            else
                dynamic_cast<TabPrintModel*>(AppAdapter::gui_app()->get_model_tab(vol_idx >= 0))->reset_model_config();
        }
        else if (col_num == colName)
        {
            if (is_windows10() && m_objects_model->HasWarningIcon(item) &&
                mouse_pos.x > 2 * app_em_unit() && mouse_pos.x < 4 * app_em_unit())
                fix_through_netfabb();
            else if (evt_context_menu)
...
void ObjectList::show_context_menu(const bool evt_context_menu)
{
    ...
    if (AppAdapter::plater()->get_current_canvas3D()->get_canvas_type() == GLCanvas3D::ECanvasType::CanvasPreview)
        return;
...
{
    wxPostEvent((wxEvtHandler*)AppAdapter::plater()->canvas3D()->get_wxglcanvas(), SimpleEvent(EVT_GLTOOLBAR_COPY));
}

void ObjectList::paste()
{
    wxPostEvent((wxEvtHandler*)AppAdapter::plater()->canvas3D()->get_wxglcanvas(), SimpleEvent(EVT_GLTOOLBAR_PASTE));
}

void ObjectList::cut()
{
    wxPostEvent((wxEvtHandler*)AppAdapter::plater()->canvas3D()->get_wxglcanvas(), SimpleEvent(EVT_GLTOOLBAR_CUT));
}

void ObjectList::clone()
{
    wxPostEvent((wxEvtHandler*)AppAdapter::plater()->canvas3D()->get_wxglcanvas(), SimpleEvent(EVT_GLTOOLBAR_CLONE));
}
...
{
	AppAdapter::plater()->undo();
}

void ObjectList::redo()
{
	AppAdapter::plater()->redo();
}
...
void ObjectList::key_event(wxKeyEvent& event)
{
    ...
    //    AppAdapter::plater()->reload_all_from_disk();
...
//#if 1
    ...
    AppAdapter::plater()->set_current_canvas_as_dirty();
}
...
void ObjectList::add_category_to_settings_from_selection(const std::vector< std::pair<std::string, bool> >& category_options, wxDataViewItem item)
{
    ...
    const DynamicPrintConfig& from_config = printer_technology() == ptFFF ?
                                            app_preset_bundle()->prints.get_edited_preset().config :
                                            app_preset_bundle()->sla_prints.get_edited_preset().config;
...
void ObjectList::add_category_to_settings_from_frequent(const std::vector<std::string>& options, wxDataViewItem item)
{
    ...
    const DynamicPrintConfig& from_config = app_preset_bundle()->prints.get_edited_preset().config;
...
void ObjectList::load_subobject(ModelVolumeType type, bool from_galery/* = false*/)
{
    ...
    wxArrayString input_files = choose_model_name(AppAdapter::gui_app()->tab_panel()->GetPage(0));
    ...
    if (type == ModelVolumeType::MODEL_PART)
        // update printable state on canvas
        AppAdapter::plater()->get_view3D_canvas3D()->update_instance_printable_state_for_object((size_t)obj_idx);
...
void ObjectList::load_modifier(const wxArrayString& input_files, ModelObject& model_object, std::vector<ModelVolume*>& added_volumes, ModelVolumeType type, bool from_galery)
{
    ...
    wxWindow* parent = AppAdapter::gui_app()->tab_panel()->GetPage(0);

    ProgressDialog dlg(_L("Loading") + dots, "", 100, AppAdapter::main_panel(), wxPD_AUTO_HIDE);
...
{
    const double side = AppAdapter::plater()->canvas3D()->get_size_proportional_to_max_bed_size(0.1);
...
void ObjectList::load_generic_subobject(const std::string& type_name, const ModelVolumeType type)
{
    ...
    AppAdapter::gui_app()->params_panel()->switch_to_object(true);

    //Show Dialog
    if (app_get("do_not_show_modifer_tips").empty()) {
        TipsDialog dlg(AppAdapter::main_panel(), _L("Add Modifier"), _L("Switch to per-object setting mode to edit modifier settings."), "do_not_show_modifer_tips");
        dlg.ShowModal();
    }
}
...
void ObjectList::switch_to_object_process()
{
    ...
    // Show Dialog
    if (app_get("do_not_show_object_process_tips").empty()) {
        TipsDialog dlg(AppAdapter::main_panel(), _L("Edit Process Settings"), _L("Switch to per-object setting mode to edit process settings of selected objects."), "do_not_show_object_process_tips");
        dlg.ShowModal();
    }
}
...
{
    const Selection& selection = AppAdapter::plater()->get_selection();
    ...
    Plater::TakeSnapshot snapshot(AppAdapter::plater(), "Add Primitive");
...
void ObjectList::load_mesh_object(const TriangleMesh &mesh, const wxString &name, bool center)
{
    ...
    Vec2d start_point = AppAdapter::plater()->build_volume().bounding_volume2d().center();

    const std::vector<Pointfs>& plate_shapes = AppAdapter::plater()->get_partplate_list().get_curr_plate()->get_shape();
...
{
    return AppAdapter::plater()->delete_object_from_model(obj_idx, refresh_immediately);
}
...
bool ObjectList::del_from_cut_object(bool is_cut_connector, bool is_model_part/* = false*/, bool is_negative_volume/* = false*/)
{
    ...
    InfoDialog dialog(AppAdapter::plater(), title,
...
void ObjectList::merge(bool to_multipart_object)
{
    ...
    if (to_multipart_object) {
        ...
        Plater::TakeSnapshot snapshot(AppAdapter::plater(), "Assemble");
    ...
    // all part's settings will be lost
        ...
        Plater::TakeSnapshot snapshot(AppAdapter::plater(), "Merge parts to an object");
...
/*void ObjectList::merge_volumes()
{
   ...
   /* AppAdapter::plater()->load_model_objects(objects);
...
#else
    AppAdapter::plater()->merge(obj_idxs[0], vol_idxs);
#endif
}*/
...
void ObjectList::boolean()
{
    ...
    Plater::TakeSnapshot snapshot(AppAdapter::plater(), "boolean");

    ModelObject* object = (*m_objects)[obj_idxs.front()];
    TriangleMesh mesh = combine_mesh_fff(*object, -1, [this](const std::string& msg) {
...
DynamicPrintConfig ObjectList::get_default_layer_config(const int obj_idx)
{
    ...
    coordf_t layer_height = object(obj_idx)->config.has("layer_height") ?
                            ...
                            app_preset_bundle()->prints.get_edited_preset().config.opt_float("layer_height");
    config.set_key_value("layer_height",new ConfigOptionFloat(layer_height));
    // BBS
    int extruder = object(obj_idx)->config.has("extruder") ?
        object(obj_idx)->config.opt_int("extruder") :
        app_preset_bundle()->prints.get_edited_preset().config.opt_float("extruder");
...
{
    AppAdapter::plater()->changed_object(obj_idx < 0 ? get_selected_obj_idx() : obj_idx);
}
...
void ObjectList::part_selection_changed()
{
            ...
            if ((type & itObject) || type == itInfo) {
                ...
                if (type == itInfo) {
                    ...
                    case InfoItemType::MmuSegmentation:
                    {
                        ...
                        //GLGizmosManager& gizmos_mgr = AppAdapter::plater()->get_view3D_canvas3D()->get_gizmos_manager();
    ...
    if (update_and_show_manipulations) {
        GLGizmosManager *gizmos_mgr = AppAdapter::plater()->get_gizmos_manager();
    ...
    if (printer_technology() == ptSLA)
        ...
        ;//AppAdapter::gui_app()->obj_layers()->get_og()->set_name(" " + og_name + " ");
    }

    update_min_height();

    Sidebar& panel = AppAdapter::gui_app()->sidebar();
    panel.Freeze();
    ...
    AppAdapter::gui_app()->obj_settings()->UpdateAndShow(update_and_show_settings);
    AppAdapter::gui_app()->obj_layers()  ->UpdateAndShow(update_and_show_layers);
...
wxDataViewItem ObjectList::add_settings_item(wxDataViewItem parent_item, const DynamicPrintConfig* config)
{
    ...
    if (is_layer_settings) {
        auto tab_object = dynamic_cast<TabPrintModel*>(AppAdapter::gui_app()->get_model_tab());
...
void ObjectList::update_info_items(size_t obj_idx, wxDataViewItemArray* selections/* = nullptr*/, bool added_object/* = false*/)
{
    ...
    // Cut connectors
    {
        ...
        if (!shows && should_show) {
            ...
            Expand(item_obj);
            if (added_object)
                get_notification_manager()->push_updated_item_info_notification(InfoItemType::CutConnectors);
...

    {
        ...
        bool should_show = printer_technology() == ptFFF
            && AppAdapter::plater()->canvas3D()->is_object_sinking(obj_idx);
        if (shows && !should_show) {
...

    {
        ...
        for (int i = 0; i < m_objects->size(); ++i) {
            if (AppAdapter::plater()->canvas3D()->is_object_sinking(i)) {
                should_show = true;
                break;
            }
            if (should_show)
...
void ObjectList::delete_from_model_and_list(const std::vector<ItemForDelete>& items_for_delete)
{
...
#endif
                AppAdapter::plater()->canvas3D()->ensure_on_bed(item->obj_idx, printer_technology() != ptSLA);
            }
            else
    ...
    if (need_update) {
        AppAdapter::plater()->update();
        AppAdapter::plater()->object_list_changed();
    }
...
void ObjectList::remove()
{
    ...
    else
    {
        ...
        Plater::TakeSnapshot snapshot = Plater::TakeSnapshot(AppAdapter::plater(), "Delete selected");
...
{
    const DynamicPrintConfig& config = app_preset_bundle()->printers.get_edited_preset().config;
...
void ObjectList::init()
{
    ...
    m_objects = &AppAdapter::gui_app()->model().objects;
}
...
{
    auto canvas_type = AppAdapter::plater()->get_current_canvas3D()->get_canvas_type();
    Selection* selection = AppAdapter::plater()->get_selection_ptr();

    const int sel_cnt = GetSelectedItemsCount();
    if (sel_cnt == 0) {
        selection->remove_all();
        if (canvas_type != GLCanvas3D::ECanvasType::CanvasPreview)
            AppAdapter::plater()->update_gizmos_on_off_state();
        return;
    }
    ...
    if (selection->contains_all_volumes(volume_idxs))
    {
        ...
        {
            Plater::TakeSnapshot snapshot(AppAdapter::plater(), "Remove selected from list", UndoRedo::SnapshotType::Selection);
    ...
    else
    {
        ...
        Plater::TakeSnapshot snapshot(AppAdapter::plater(), "Add selected to list", UndoRedo::SnapshotType::Selection);
...
void ObjectList::select_items(const wxDataViewItemArray& sels)
{
    ...
    if (!sels.empty()) {
        ...
        int curr_plate_idx = AppAdapter::plater()->get_partplate_list().get_curr_plate_index();
...
{
    if (AppAdapter::plater()->get_gizmos_manager()->get_current_type() == GLGizmosManager::Scale) {
        for (const auto &item : sels) {
...
void ObjectList::update_settings_item_and_selection(wxDataViewItem item, wxDataViewItemArray& selections)
{
    ...
    if (selections.Index(old_settings_item) != wxNOT_FOUND)
    {
        ...
        if (old_settings_item == new_settings_item)
        {
            ...
            AppAdapter::gui_app()->obj_settings()->UpdateAndShow(true);
...
void ObjectList::instances_to_separated_object(const int obj_idx, const std::set<int>& inst_idxs)
{
    ...
    AppAdapter::plater()->get_view3D_canvas3D()->update_instance_printable_state_for_object(new_obj_indx);
...
void ObjectList::instances_to_separated_objects(const int obj_idx)
{
    ...
    // update printable state for new volumes on canvas3D
    AppAdapter::plater()->get_view3D_canvas3D()->update_instance_printable_state_for_objects(object_idxs);
...
void ObjectList::split_instances()
{
    ...
    Plater::TakeSnapshot snapshot(AppAdapter::plater(), "Instances to Separated Objects");
...
void ObjectList::rename_item()
{
    ...
    if (has_illegal_filename_characters(new_name)) {
...
void ObjectList::fix_through_netfabb()
{
    ...
    if (!AppAdapter::plater()->get_gizmos_manager()->check_gizmos_closed_except(GLGizmosManager::Undefined))
        return;
                                          ...
                                          std::vector<std::pair<std::string, std::string>>& failed_models)
    {
        ...
        if (!fix_model_by_win10_sdk_gui(*(object(obj_idx)), vol_idx, progress_dlg, msg, res))
            return false;
        //AppAdapter::plater()->changed_mesh(obj_idx);
...
{
    auto plater = AppAdapter::plater();
    GLGizmosManager* gizmos_mgr = plater->get_gizmos_manager();
...
void ObjectList::set_extruder_for_selected_items(const int extruder)
{
    ...
    std::vector<std::string> colors = AppAdapter::plater()->get_extruder_colors_from_plater_config();
...
void ObjectList::toggle_printable_state()
{
    ...
    // update scene
    AppAdapter::plater()->update();
    AppAdapter::plater()->reload_paint_after_background_process_apply();
}
