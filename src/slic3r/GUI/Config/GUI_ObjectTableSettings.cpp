...
#include "slic3r/GUI/Config/GUI_ObjectTableSettings.hpp"
    ...
    for (auto& cat : cat_options)
    {
        ...
        auto extra_column = [this, is_object, object, config, group_category](wxWindow* parent, const Line& line)
        {
            ...
            btn->Bind(wxEVT_BUTTON, [btn, opt_key, this, is_object, object, config, group_category](wxEvent &event) {
                //AppAdapter::plater()->take_snapshot(from_u8((boost::format(_utf8(L("Reset Option %s"))) % opt_key).str()));
                config->erase(opt_key);
                //btn->Hide();
                AppAdapter::obj_list()->changed_object();
        ...
        for (auto& opt : cat.second)
        {
            ...
            option.opt.width = 18;
            if (is_extruders_cat)
                option.opt.max = preset_extruders_edited_cnt();
        ...
        for (auto& opt : cat.second)
            optgroup->get_field(opt.name)->m_on_change = [weak_optgroup](const std::string& opt_id, const boost::any& value) {
                ...
                AppAdapter::plater()->take_snapshot((boost::format("Change Option %s") % opt_id).str());
        ...
        optgroup->update_visibility((ConfigOptionMode)app_get_mode());
    }
...
{
    bool is_added = false;
    if (AppAdapter::plater()->printer_technology() == ptFFF)
    {
...
void ObjectTableSettings::update_config_values(bool is_object, ModelObject* object, ModelConfig* config, const std::string& category)
{
    ...
    for (auto og : m_og_settings) {
        og->update_visibility((ConfigOptionMode)app_get_mode());
    }
    m_parent->Layout();
    m_parent->Fit();
