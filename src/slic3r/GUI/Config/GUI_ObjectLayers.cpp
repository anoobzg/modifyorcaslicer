...
{
    m_grid_sizer = new wxFlexGridSizer(5, 0, app_em_unit()); // Title, Min Z, "to", Max Z, unit & buttons sizer
...
wxSizer* ObjectLayers::create_layer(const t_layer_height_range& range, PlusMinusButton *delete_button, PlusMinusButton *add_button) 
{
    ...
    head_text->SetFont(app_normal_font());
        ...
        [range, update_focus_data, this, delete_button, add_button](coordf_t min_z, bool enter_pressed, bool dont_update_ui) 
    {
        ...
        return AppAdapter::obj_list()->edit_layer_range(range, new_range, dont_update_ui);
    });
    ...
    middle_text->SetFont(app_normal_font());
    ...
    unit_text->SetFont(app_normal_font());
    ...
    //{
    //    return AppAdapter::obj_list()->edit_layer_range(range, layer_height);
    //});
    ...
    //sizer->Add(temp, 0, wxLEFT | wxALIGN_CENTER_VERTICAL, app_em_unit());
...
void ObjectLayers::create_layers_list()
{
    for (const auto &layer : m_object->layer_config_ranges) {
        ...
        wxString tooltip = AppAdapter::obj_list()->can_add_new_range_after_current(range);
        ...
        del_btn->Bind(wxEVT_BUTTON, [del_btn](wxEvent &) {
            AppAdapter::obj_list()->del_layer_range(del_btn->range);
        });

        add_btn->Bind(wxEVT_BUTTON, [add_btn](wxEvent &) {
            AppAdapter::obj_list()->add_layer_range_after_current(add_btn->range);
        });
    }
}
...
{
    ObjectList* objects_ctrl   = AppAdapter::obj_list();
    ...
    AppAdapter::app()->CallAfter([this, type, objects_ctrl, range]() {
...
void ObjectLayers::update_scene_from_editor_selection() const
{
    ...
    AppAdapter::plater()->canvas3D()->handle_layers_data_focus_event(m_selectable_range, m_selection_type);
}
...
void ObjectLayers::msw_rescale()
{
    ...
    m_grid_sizer->SetHGap(app_em_unit());
...
{
    SetMinSize(wxSize(app_em_unit(), wxDefaultCoord));
}
