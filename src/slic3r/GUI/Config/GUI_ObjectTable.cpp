...
void GridCellChoiceEditor::Create(wxWindow *parent, wxWindowID id, wxEvtHandler *evtHandler)
{
    ...
    bitmap_combo->SetFont(Font::Body_12);
    int         array_count  = m_choices.GetCount();
    for (int i = 0; i < array_count; i++) {
                              ...
                              bool isSelected)
{
    ...
    if (table) {
        ...
        grid.GetTextExtent(L("Support Disabled"), &text_width, &text_height, NULL, NULL, &Font::Body_10);
...
void ObjectGridTable::update_value_to_object(Model* model, ObjectGridRow* grid_row, int col)
{
    ...
    if (grid_row->row_type != row_object) {
       ...
       /* else if (col == col_assemble_name) {
            ...
            AppAdapter::obj_list()->printable_state_changed(object_volume_ids);
    ...
    if ((name_ptr) && (*name_ptr != name_value)) {
        ...
        *name_ptr = name_value;
        //notify object list
        AppAdapter::obj_list()->update_name_for_items();
    }
}
...
void ObjectGridTable::SetValue( int row, int col, const wxString& value )
{
    ...
    ObjectList* obj_list = AppAdapter::obj_list();
    if (grid_col->type == coEnum) {
        int enum_value = 0;
        ...
        if (col == col_brim_type) {
            ...
            AppAdapter::obj_list()->update_filament_values_for_items(m_panel->m_filaments_count);
...
{
    const float font_size = 1.5f * app_em_unit();
...
void ObjectGridTable::reload_cell_data(int row, const std::string& category)
{
    ...
    AppAdapter::obj_list()->object_config_options_changed(object_volume_id);
    if (object_volume_id.volume)
        dynamic_cast<TabPrintModel*>(AppAdapter::gui_app()->get_model_tab(true))->update_model_config();
    else
        dynamic_cast<TabPrintModel*>(AppAdapter::gui_app()->get_model_tab())->update_model_config();
    //m_panel->m_plater->update();
}
    ...
    for (int col = 1; col < col_speed_perimeter_reset; col++)
    {
        ...
        for (int row = 1; row < get_row_count(); row++)
        {
            ...
            if ((!grid_col->b_icon) && (grid_col->b_for_object)) {
                ...
                grid_table->SetCellFont(row, col, Font::Body_12);
                /*if (grid_row->row_type == ObjectGridTable::row_object)
                    grid_table->SetCellFont(row, col, Font::Head_14);
                else
                    grid_table->SetCellFont(row, col, Font::Body_14);*/
            }
        }
    }
...
void ObjectGridTable::OnSelectCell(int row, int col)
{
    ...
    if (row == 0 || col == col_filaments) {
        ...
        AppAdapter::obj_list()->select_items(object_volume_ids);
    }
                    ...
                    : wxPanel( parent, id, pos, size, style, name ), m_model(modelObj), m_plater(platerObj), m_float_validator(2, nullptr, wxNUM_VAL_ZERO_AS_BLANK)
{
    ...
    /* m_page_text = new wxStaticText(m_side_window, wxID_ANY, wxString(L("Per Object Setting")), wxDefaultPosition, wxSize(-1, 32), wxALIGN_CENTRE_HORIZONTAL|wxST_ELLIPSIZE_END);
     ...
     m_page_text->SetFont(Font::Head_18);
    ...
    m_side_window->SetFont(Font::Body_12);
...
int ObjectTablePanel::init_filaments_and_colors()
{
    ...
    const std::vector<std::string> filament_presets = app_preset_bundle()->filament_presets;
...
void ObjectTablePanel::load_data()
{
    ...
    m_object_grid->SetLabelFont(Font::Head_13);
        ...
        for (int row = 1; row < rows; row++)
        {
            ...
            //}
                ...
                if (col == ObjectGridTable::col_name) {
                    m_object_grid->SetCellFont(row, col, Font::Body_13);
                    /* if (grid_row->row_type == ObjectGridTable::row_object)
                         m_object_grid->SetCellFont(row, col, Font::Head_14);
                     else
                         m_object_grid->SetCellFont(row, col, Font::Body_14);*/
                }
            }
        }
    }
    ...
    m_model(modelObj), m_plater(platerObj)
{
    ...
    //m_static_title->SetFont(Font::Head_12);
    ...
    // icon
    std::string icon_path = (boost::format("%1%/images/main.ico") % resources_dir()).str();
