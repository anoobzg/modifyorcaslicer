...
static void call_after_if_active(std::function<void()> fn, GUI_App* app = AppAdapter::gui_app())
{
    // check application GUI
    if (app == nullptr) return;
    AppAdapter::app()->CallAfter([fn, app]() {
        // app must exist because it call this
        // if (app == nullptr) return;
        Plater *plater = app->plater();
        if (plater == nullptr) return;
        GLGizmosManager *mng = plater->get_gizmos_manager();
...
GLGizmoSimplify::GLGizmoSimplify(GLCanvas3DFacade*       parent,
    ...
    for (size_t object_id : big_ids) {
        ...
        std::function<bool(wxEvtHandler *)> open_simplify =
            [object_id](wxEvtHandler *) {
                ...
                auto manager = plater->get_gizmos_manager();
...
void GLGizmoSimplify::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    const ModelVolume *act_volume = get_model_volume(selection, AppAdapter::plater()->model());
    if (act_volume == nullptr) {
        stop_worker_thread_request();
        close();
        if (! m_parent->get_selection()->is_single_volume()) {
            MessageDialog msg((wxWindow*)AppAdapter::main_panel(),
...
void GLGizmoSimplify::apply_simplify() {
    ...
    ModelVolume* mv = get_model_volume(selection, AppAdapter::gui_app()->model());
    assert(mv == m_volume);
    ...
    AppAdapter::obj_list()->update_item_error_icon(object_idx, -1);
    close();
}
...
void GLGizmoSimplify::on_render()
{
    ...
    if (m_volume != get_model_volume(selection, AppAdapter::gui_app()->model()))
        return;

    const Transform3d trafo_matrix = selected_volume->world_matrix();
    auto* gouraud_shader = get_shader("gouraud_light");
    ...
    if (m_show_wireframe) {
        auto* contour_shader = get_shader("mm_contour");
        contour_shader->start_using();
        contour_shader->set_uniform("offset", is_mesa() ? 0.0005 : 0.00001);
