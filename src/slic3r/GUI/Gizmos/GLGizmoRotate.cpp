...
GLGizmoRotate::GLGizmoRotate(GLCanvas3DFacade* parent, GLGizmoRotate::Axis axis)
...
GLGizmoRotate3D::GLGizmoRotate3D(GLCanvas3DFacade* parent, const std::string& icon_filename, unsigned int sprite_id, GizmoObjectManipulation* obj_manipulation)
...
bool GLGizmoRotate3D::on_mouse(const wxMouseEvent &mouse_event)
{
    if (mouse_event.Dragging() && m_dragging) {
        ...
        if (m_parent->get_selection()->is_wipe_tower())
            ...
            switch (AppAdapter::gui_app()->obj_manipul()->get_coordinates_type())
            {
            default:
...
void GLGizmoRotate3D::data_changed(bool is_serializing) {
    ...
    if (is_wipe_tower) {
        DynamicPrintConfig& config = app_preset_bundle()->prints.get_edited_preset().config;
