...
void gizmo_update_render_colors()
{
...
{
    AppAdapter::plater()->canvas3D()->remove_raycasters_for_picking(SceneRaycaster::EType::Gizmo, picking_id);
    picking_id = -1;
    raycasters = { nullptr };
}
...
{
    GLShaderProgram* shader = get_current_shader();
    if (shader == nullptr)
        return;
...
GLGizmoBase::GLGizmoBase(GLCanvas3DFacade* parent, const std::string& icon_filename, unsigned int sprite_id)
    : m_parent(parent)
    , m_group_id(-1)
    , m_state(Off)
...
//                m_grabbers[m_hover_id].dragging = true;
        ...
        if (mouse_event.Dragging()) {
            ...
            AppAdapter::gui_app()->obj_manipul()->set_dirty();
            m_parent->set_as_dirty();
            return true;
        }
