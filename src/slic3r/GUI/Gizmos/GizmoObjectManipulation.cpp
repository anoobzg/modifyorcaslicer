...
static double get_volume_min_z(const GLVolume* volume)
{
    ...
    const ModelObject* mo = AppAdapter::gui_app()->model().objects[volume->composite_id.object_id];
...
GizmoObjectManipulation::GizmoObjectManipulation(GLCanvas3DFacade* glcanvas)
    : m_glcanvas(glcanvas)
{
    m_imperial_units = AppAdapter::app_config()->get("use_inches") == "1";
...
{
    if (m_imperial_units != (AppAdapter::app_config()->get("use_inches") == "1")) {
        m_imperial_units = AppAdapter::app_config()->get("use_inches") == "1";
...
void GizmoObjectManipulation::update_settings_value(const Selection* selection)
{
    ...
    if (selection->is_single_full_instance()) {
        ...
        if (is_world_coordinates()) {
			...
			m_new_rotation = volume->get_instance_rotation() * (180. / M_PI);
			m_new_size     = volume->get_instance_transformation().get_scaling_factor().cwiseProduct(AppAdapter::gui_app()->model().objects[volume->object_idx()]->raw_mesh_bounding_box().size());
...
void GizmoObjectManipulation::change_size_value(int axis, double value)
{
    ...
    if (selection->is_single_volume() || selection->is_single_modifier()) {
        Vec3d instance_scale = AppAdapter::gui_app()->model().objects[selection->get_first_volume()->object_idx()]->instances[0]->get_transformation().get_scaling_factor();
        ...
        ref_size = is_world_coordinates() ? 
            ...
            AppAdapter::gui_app()->model().objects[selection->get_first_volume()->object_idx()]->raw_mesh_bounding_box().size();
...
{ 
    ...
    if (selection->is_single_full_instance() && is_world_coordinates() && !new_value) {
		...
		if (! Geometry::is_rotation_ninety_degrees(volume->get_instance_rotation())) {
            ...
            AppAdapter::gui_app()->model().objects[volume->composite_id.object_id]->bake_xy_rotation_into_meshes(volume->composite_id.instance_id);
...
{
    Plater::TakeSnapshot snapshot(AppAdapter::plater(), "Reset scale");
