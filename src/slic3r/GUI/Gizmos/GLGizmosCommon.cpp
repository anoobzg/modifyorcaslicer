...
CommonGizmosDataPool::CommonGizmosDataPool(GLCanvas3DFacade* canvas)
    : m_canvas(canvas)
{
...
void InstancesHider::on_update()
{
    ...
    GLCanvas3DFacade* canvas = get_pool()->get_canvas();
    double z_min = -SINKING_Z_THRESHOLD;
...
void ObjectClipper::set_position_by_ratio(double pos, bool keep_normal, bool vertical_normal)
{
    ...
    if(vertical_normal) {
        ...
        //Vec3d camera_dir = AppAdapter::plater()->get_camera().get_dir_forward();
        //if (abs(camera_dir(0)) > EPSILON || abs(camera_dir(1)) > EPSILON)
        //    camera_dir(2) = 0;

        normal = (keep_normal && m_clp) ? m_clp->get_normal() : /*-camera_dir;*/ -AppAdapter::plater()->get_camera().get_dir_forward();
    }
    Vec3d center = mo->instances[active_inst]->get_offset() + Vec3d(0., 0., z_shift);
...
AssembleViewDataPool::AssembleViewDataPool(GLCanvas3DFacade* canvas)
    : m_canvas(canvas)
{
...
{
    Vec3d normal = (keep_normal && m_clp) ? m_clp->get_normal() : -AppAdapter::plater()->get_camera().get_dir_forward();
