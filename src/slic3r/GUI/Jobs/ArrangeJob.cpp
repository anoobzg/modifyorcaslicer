...
// Cache the wti info
class WipeTower: public WipeTowerInfoHelper {
public:
    explicit WipeTower(const WipeTowerInfoHelper &wti)
        : WipeTowerInfoHelper(wti)
    {}

    explicit WipeTower(WipeTowerInfoHelper &&wti)
        : WipeTowerInfoHelper(std::move(wti))
    {}
...
ArrangePolygon ArrangeJob::prepare_arrange_polygon(void* model_instance)
{
    ...
    const Slic3r::DynamicPrintConfig& config = app_preset_bundle()->full_config();
...
void ArrangeJob::prepare_wipe_tower()
{
    ...
    DynamicPrintConfig& current_config = app_preset_bundle()->prints.get_edited_preset().config;
...
void ArrangeJob::prepare()
{
    ...
    const Slic3r::DynamicPrintConfig& config = app_preset_bundle()->full_config();
    int numExtruders = app_preset_bundle()->filament_presets.size();

    Model::setExtruderParams(config, numExtruders);
    Model::setPrintSpeedTable(config, PrintConfig());
    ...
    { // subtract excluded region and get a polygon bed
        auto& print = AppAdapter::plater()->get_partplate_list().get_current_fff_print();
...
void ArrangeJob::process(Ctl &ctl)
{
    ...
    const Slic3r::DynamicPrintConfig& global_config = app_preset_bundle()->full_config();
...
ArrangeJob::ArrangeJob() : m_plater{AppAdapter::plater()} { }
...
arrangement::ArrangeParams init_arrange_params(Plater *p)
{
    ...
    double object_skirt_offset = 0.0;
    double object_skirt_witdh = 0.0;
