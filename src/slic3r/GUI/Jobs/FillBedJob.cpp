...
void FillBedJob::process(Ctl &ctl)
{
    ...
    Points plate_shape = Points();
    const std::vector<Pointfs>& plate_shapes = m_plater->get_partplate_list().get_curr_plate()->get_shape();
    if (!plate_shapes.empty()) {
        Point center = m_selected.front().transformed_poly().contour.centroid();
        plate_shape = arrangement::get_plate_shape(plate_shapes, center);
    }
    m_bedpts = get_shrink_bedpts(m_plater->config(), params, plate_shape);
...
FillBedJob::FillBedJob() : m_plater{AppAdapter::plater()} {}
