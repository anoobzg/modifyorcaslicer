...
wxDEFINE_EVENT(EVT_WIPE_TOWER_RammingChart_CHANGED, wxCommandEvent);

void RammingChart::draw() {
    ...
    dc.SetBrush(wxBrush(Slic3r::GUI::get_highlight_default_clr()));
#else
...
void RammingChart::mouse_right_button_clicked(wxMouseEvent& event) {
...
void RammingChart::mouse_clicked(wxMouseEvent& event) {
    wxPoint point = event.GetPosition();
    int button_index = which_button_is_clicked(point);
    if ( button_index != -1) {
...
void RammingChart::mouse_moved(wxMouseEvent& event) {
    if (!event.Dragging() || !m_dragged) return;
    wxPoint pos = event.GetPosition();    
    wxRect rect = m_rect;
    rect.Deflate(side/2.);
    if (!(rect.Contains(pos))) {  // the mouse left RammingChart area
...
void RammingChart::mouse_double_clicked(wxMouseEvent& event) {
    if (!manual_points_manipulation)
        return;
    wxPoint point = event.GetPosition();
    if (!m_rect.Contains(point))     // the click is outside the RammingChart
        return;
...
void RammingChart::recalculate_line() {
    ...
    wxPostEvent(this->GetParent(), wxCommandEvent(EVT_WIPE_TOWER_RammingChart_CHANGED));
    Refresh();
}



std::vector<float> RammingChart::get_ramming_speed(float sampling) const {
...
std::vector<std::pair<float,float>> RammingChart::get_buttons() const {
...
BEGIN_EVENT_TABLE(RammingChart, wxWindow)
EVT_MOTION(RammingChart::mouse_moved)
EVT_LEFT_DOWN(RammingChart::mouse_clicked)
EVT_LEFT_UP(RammingChart::mouse_released)
EVT_LEFT_DCLICK(RammingChart::mouse_double_clicked)
EVT_RIGHT_DOWN(RammingChart::mouse_right_button_clicked)
EVT_LEAVE_WINDOW(RammingChart::mouse_left_window)
EVT_PAINT(RammingChart::paint_event)
END_EVENT_TABLE()
