...
#include "slic3r/GUI/Slice/2DBed.hpp"
...
BedShapeDialog::BedShapeDialog(wxWindow* parent) 
...
void BedShapePanel::build_panel(const ConfigOptionPoints& default_pt, const std::string& custom_texture, const std::string& custom_model)
{
    ...
    sbsizer->GetStaticBox()->SetFont(app_bold_font());
    UpdateDarkUI(sbsizer->GetStaticBox());

	// shape options
    m_shape_options_book = new wxChoicebook(this, wxID_ANY, wxDefaultPosition, wxSize(25*app_em_unit(), -1), wxCHB_TOP);
    UpdateDarkUI(m_shape_options_book->GetChoiceCtrl());
...
wxPanel* BedShapePanel::init_texture_panel()
{
    ...
    line.widget = [this](wxWindow* parent) {
        ...
        UpdateDarkUI(load_btn);
        UpdateDarkUI(remove_btn);
        ...
        filename_lbl->Bind(wxEVT_UPDATE_UI, ([this](wxUpdateUIEvent& e) {
                ...
                if (lbl != nullptr) {
                    ...
                    lbl->SetForegroundColour(exists ? get_label_clr_default() : wxColor(*wxRED));
