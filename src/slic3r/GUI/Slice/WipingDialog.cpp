...
#include "slic3r/GUI/Widgets/CommonDIPDef.hpp"
...
int scale_wiping(const int val) { return val * app_em_unit() / 10; }
int ITEM_WIDTH_WIPPING() { return scale_wiping(30); }
                ...
                wxDEFAULT_DIALOG_STYLE /* | wxRESIZE_BORDER*/)
{
    ...
    auto sizer_width = (int)((sqrt(matrix.size()) + 2.8)*ITEM_WIDTH_WIPPING());
    ...
    if (this->FindWindowById(wxID_CANCEL, this)) {
        UpdateDarkUI(static_cast<wxButton*>(this->FindWindowById(wxID_CANCEL, this)));
...
void WipingPanel::create_panels(wxWindow* parent, const int num) {
    for (size_t i = 0; i < num; i++)
    {
        ...
        for (int j = 0; j < num; ++j) {
            ...
            edit_boxes[j][i]->SetFont(Font::Body_13);
...
,m_matrix(matrix), m_min_flush_volume(extra_flush_volume), m_max_flush_volume(Slic3r::g_max_flush_volume)
{
    ...
    auto sizer_width = (int)((sqrt(matrix.size())) * ITEM_WIDTH_WIPPING() + (sqrt(matrix.size()) + 1) * HEADER_BEG_PADDING);
    sizer_width = sizer_width > MIN_WIPING_DIALOG_WIDTH ? sizer_width : MIN_WIPING_DIALOG_WIDTH;
    ...
    UpdateDarkUI(m_page_simple);
    UpdateDarkUI(m_page_advanced);
...
#ifdef _WIN32
            wxTextCtrl* text = new wxTextCtrl(m_page_advanced, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(ITEM_WIDTH_WIPPING(), -1), wxTE_CENTER | wxBORDER_NONE | wxTE_PROCESS_ENTER);
            UpdateDarkUI(text);
            edit_boxes.back().push_back(text);
#else
            edit_boxes.back().push_back(new wxTextCtrl(m_page_advanced, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(ITEM_WIDTH_WIPPING(), -1)));
#endif
            if (i == j) {
    ...
    tip_message_panel->SetSizer(message_sizer);
    {
        ...
        m_tip_message_label->SetFont(Font::Body_13);
    ...
    bool is_show = app_get_bool("auto_calculate") || app_get_bool("auto_calculate_when_filament_change");
    ...
    // BBS: for tunning flush volumes
    {
        ...
        auto on_apply_text_modify = [this](wxEvent& e) {
            ...
            for (unsigned int i = 0; i < m_number_of_extruders; ++i) {
                for (unsigned int j = 0; j < m_number_of_extruders; ++j) {
                    edit_boxes[i][j]->SetValue(std::to_string(int(m_matrix[m_number_of_extruders * j + i] * multiplier)));
                }
            }
    ...
    {
        wxSpinCtrl* spin_ctrl = new wxSpinCtrl(m_page_simple, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(ITEM_WIDTH_WIPPING(), -1), style | wxALIGN_RIGHT, 0, 300, (int)initial);
        UpdateDarkUI(spin_ctrl);
...
{
    auto& ams_multi_color_filament = app_preset_bundle()->ams_multi_color_filment;
