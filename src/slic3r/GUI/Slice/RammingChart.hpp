...
wxDECLARE_EVENT(EVT_WIPE_TOWER_RammingChart_CHANGED, wxCommandEvent);

class RammingChart : public wxWindow {
        
public:
    RammingChart(wxWindow* parent, wxRect rect,const std::vector<std::pair<float,float>>& initial_buttons,int ramming_speed_size, float sampling, int scale_unit=10) :
