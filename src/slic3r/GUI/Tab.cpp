...
#include "slic3r/Config/PresetHints.hpp"
#include "libslic3r/PresetBundle.hpp"
#include "libslic3r/PrintConfig.hpp"
#include "libslic3r/Utils.hpp"
#include "libslic3r/Model.hpp"
#include "libslic3r/GCode/GCodeProcessor.hpp"
#include "slic3r/GUI/Slice/RammingDialog.hpp"
#include "slic3r/Config/AppPreset.hpp"
#include "slic3r/Config/Search.hpp"
#include "slic3r/Config/OG_CustomCtrl.hpp"
...
#include "slic3r/Config/OptionsGroup.hpp"
...
#include "slic3r/GUI/Dialog/SavePresetDialog.hpp"
#include "slic3r/GUI/Dialog/EditGCodeDialog.hpp"
...
#include "slic3r/GUI/Slice/BedShapeDialog.hpp"
    ...
    m_parent(parent), m_title(title), m_type(type)
{
    ...
    this->SetFont(Slic3r::GUI::app_normal_font());
    ...
    AppAdapter::gui_app()->tabs_list.push_back(this);

    m_em_unit = em_unit(m_parent); //app_em_unit();
...
/*#ifdef __WINDOWS__
    ...
    m_preset_bundle = app_preset_bundle();
    ...
    if (m_type < Preset::TYPE_COUNT) {
        ...
        // m_presets_choice->SetFont(Font::Body_10); // BBS
    ...
    //StateColor::darkModeColorFor(wxColour(238, 238, 238)), wxDefaultPosition, wxSize(m_top_panel->GetSize().GetWidth(), 3 * app_em_unit()), 8);
    ...
    m_search_input->SetFont(app_bold_font());
    ...
    m_btn_search->Bind(
        ...
        [this](wxCommandEvent &) {
         ...
         AppAdapter::plater()->search(false, m_type, m_top_panel->GetParent(), m_search_input, m_btn_search);
         Thaw();
    ...
    /* m_search_btn->Bind(wxEVT_BUTTON, [](wxCommandEvent) { AppAdapter::plater()->search(false); });*/
    ...
    const float scale_factor = /*AppAdapter::gui_app()->*/em_unit(this)*0.1;// GetContentScaleFactor();
    ...
    if (dynamic_cast<TabPrint*>(this) == nullptr) {
        m_static_title = new Label(m_top_panel, Font::Body_12, _L("Advance"));
    ...
    m_tabctrl->SetFont(Font::Body_14);
...
void Tab::load_initial_data()
{
    ...
    if(m_presets->get_selected_preset_parent() != nullptr)
    {
        m_bmp_non_system = &m_bmp_value_unlock;
        m_ttg_non_system = &m_ttg_value_unlock;
        m_tt_non_system  = &m_tt_value_unlock;
...
{
    m_default_text_clr = get_label_clr_default();
    if (m_sys_label_clr == get_label_clr_sys() && m_modified_label_clr == get_label_clr_modified())
        return;
    m_sys_label_clr = get_label_clr_sys();
    m_modified_label_clr = get_label_clr_modified();
...
void Tab::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    ConfigOptionsGroup* og_freq_chng_params = AppAdapter::gui_app()->sidebar().og_freq_chng_params(is_fff);
    //BBS: GUI refactor
    ...
    if (opt_key == "single_extruder_multi_material"  ){
        ...
        AppAdapter::gui_app()->sidebar().show_SEMM_buttons(bSEMM);
    ...
    if (opt_key == "enable_prime_tower") {
        ...
        if (!boost::any_cast<bool>(value) && timelapse_enabled) {
            MessageDialog dlg(AppAdapter::plater(), _L("Prime tower is required for smooth timelapse. There may be flaws on the model without prime tower. Are you sure you want to disable prime tower?"),
    ...
    if (opt_key == "timelapse_type") {
        ...
        if (!wipe_tower_enabled && boost::any_cast<int>(value) == (int)TimelapseType::tlSmooth) {
            MessageDialog dlg(AppAdapter::plater(), _L("Prime tower is required for smooth timelapse. There may be flaws on the model without prime tower. Do you want to enable prime tower?"),
                              _L("Warning"), wxICON_WARNING | wxYES | wxNO);
    ...
    if (opt_key == "print_sequence" && m_config->opt_enum<PrintSequence>("print_sequence") == PrintSequence::ByObject) {
        ...
        if (printer_structure_opt && printer_structure_opt->value == PrinterStructure::psI3) {
            ...
            MessageDialog dialog(AppAdapter::plater(), msg_text, "", wxICON_WARNING | wxYES | wxNO);
    ...
    if (opt_key == "support_interface_filament") {
        ...
        if (is_support_filament(interface_filament_id) && !(m_config->opt_float("support_top_z_distance") == 0 && m_config->opt_float("support_interface_spacing") == 0 &&
            ...
            MessageDialog      dialog(AppAdapter::plater(), msg_text, "Suggestion", wxICON_WARNING | wxYES | wxNO);
            DynamicPrintConfig new_conf = *m_config;
    ...
    if(opt_key=="layer_height"){
        auto min_layer_height_from_nozzle=app_preset_bundle()->full_config().option<ConfigOptionFloats>("min_layer_height")->values;
        auto max_layer_height_from_nozzle=app_preset_bundle()->full_config().option<ConfigOptionFloats>("max_layer_height")->values;
        ...
        if (exceed_maximum_flag || exceed_minimum_flag) {
            if (lh < EPSILON) {
                ...
                MessageDialog dialog(AppAdapter::plater(), msg_text, "", wxICON_WARNING | wxOK);
    ...
    auto update_flush_volume = [](int idx = -1) {
        if (idx < 0) {
            size_t filament_size = AppAdapter::plater()->get_extruder_colors_from_plater_config().size();
            for (size_t i = 0; i < filament_size; ++i)
                AppAdapter::plater()->sidebar().auto_calc_flushing_volumes(i);
        }
        else
    ...
    if (opt_key_without_idx == "long_retractions_when_cut") {
        ...
        if (activate == 1) {
            MessageDialog dialog(AppAdapter::plater(),
    ...
    if (opt_key == "extruders_count" && !m_config->opt_bool("single_extruder_multi_material")){
        ...
        int         old_filament_size = app_preset_bundle()->filament_presets.size();
        ...
        app_preset_bundle()->set_num_filaments(num_extruder, new_colors);
        AppAdapter::plater()->on_filaments_change(num_extruder);
        AppAdapter::gui_app()->get_tab(Preset::TYPE_PRINT)->update();
...
void Tab::update_wiping_button_visibility() {
    ...
    // auto wiping_dialog_button = AppAdapter::gui_app()->sidebar().get_wiping_dialog_button();
...
void Tab::activate_option(const std::string& opt_key, const wxString& category)
{
    ...
    //BBS: GUI refactor
    //AppAdapter::main_panel()->select_tab(this);
    AppAdapter::main_panel()->select_tab((wxPanel*)m_parent);
...
{
    AppAdapter::gui_app()->sidebar().get_searcher().apply(m_config, m_type, m_mode);
}
...
void Tab::on_presets_changed()
{
    ...
    AppAdapter::plater()->sidebar().update_presets(m_type);

    PlateBed::set_render_option(false, true, false);

    // Printer selected at the Printer tab, update "compatible" marks at the print and filament selectors.
    for (auto t: m_dependent_tabs)
    {
        Tab* tab = AppAdapter::gui_app()->get_tab(t);
...
void Tab::build_preset_description_line(ConfigOptionsGroup* optgroup)
{
    ...
    auto detach_preset_btn = [this](wxWindow* parent) {
        ...
        btn->SetFont(Slic3r::GUI::app_normal_font());
...
{
    const bool is_fff = supports_printer_technology(ptFFF);
    auto og_freq_chng_params = AppAdapter::gui_app()->sidebar().og_freq_chng_params(is_fff);
...
void TabPrint::update()
{
    ...
    if (m_update_cnt==0) {
        ...
        if (m_type != Preset::TYPE_MODEL && !AppAdapter::plater()->inside_snapshot_capture())
            AppAdapter::obj_list()->update_and_show_object_settings_item();

        AppAdapter::main_panel()->on_config_changed(m_config);
    }
}
...
void TabPrintModel::update_model_config()
{
    ...
    if (m_type != Preset::TYPE_PLATE) {
        m_config->apply_only(*AppAdapter::gui_app()->plate_tab->get_config(), plate_keys);
    }
    m_null_keys.clear();
        ...
        if (m_type == Preset::Type::TYPE_PLATE) {
            ...
            for (auto plate_item : m_object_configs) {
                ...
                if (!plate_config.has("curr_bed_type")) {
                    // same as global
                    DynamicConfig& global_cfg = app_preset_bundle()->project_config;
...
void TabPrintModel::reset_model_config()
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Reset Options"));
    for (auto config : m_object_configs) {
...
void TabPrintModel::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    if (!has_key(k))
        return;
    if (!m_object_configs.empty())
        AppAdapter::plater()->take_snapshot((boost::format("Change Option %s") % k).str());
...
{
    m_parent_tab = AppAdapter::gui_app()->get_tab(Preset::TYPE_PRINT);
            ...
            if (k == "first_layer_sequence_choice") {
                ...
                if (first_layer_seq_choice == LayerSeq::flsAuto) {
                    ...
                    if (!plate_config.has("first_layer_print_sequence")) {
                        ...
                        for (int i = 0; i < preset_filaments_cnt(); i++) {
                    ...
                    evt.SetEventObject(AppAdapter::plater());
...
void TabPrintPlate::notify_changed(ObjectBase* object)
{
    ...
    auto objects_list = AppAdapter::obj_list();
...
void TabPrintObject::notify_changed(ObjectBase * object)
{
    ...
    AppAdapter::obj_list()->object_config_options_changed({obj, nullptr});
}
...
{
    m_parent_tab = AppAdapter::gui_app()->get_model_tab();
}

void TabPrintPart::notify_changed(ObjectBase * object)
{
    auto vol = dynamic_cast<ModelVolume*>(object);
    AppAdapter::obj_list()->object_config_options_changed({vol->get_object(), vol});
}
    ...
    if (invalid) {
        ...
        wxString reports = format_wxstr(
            _L_PLURAL("Following line %s contains reserved keywords.\nPlease remove it, or will beat G-code visualization and printing time estimation.",
                      ...
                      tags.size()),
            lines);
        //wxMessageDialog dialog(AppAdapter::main_panel(), reports, _L("Found reserved keywords in") + " " + _(title), wxICON_WARNING | wxOK);
        MessageDialog dialog(AppAdapter::main_panel(), reports, _L("Reserved keywords found") + " " + _(title), wxICON_WARNING | wxOK);
        dialog.ShowModal();
    }
    return !invalid;
}
...
void TabFilament::build()
{
    ...
    auto page = add_options_page(L("Filament"), "custom-gcode_filament"); // ORCA: icon only visible on placeholders
        ...
        optgroup->m_on_change = [this, optgroup](t_config_option_key opt_key, boost::any value) {
            DynamicPrintConfig &filament_config = app_preset_bundle()->filaments.get_edited_preset().config;
    ...
    page = add_options_page(L("Multimaterial"), "custom-gcode_multi_material"); // ORCA: icon only visible on placeholders
        ...
        create_line_with_widget(optgroup.get(), "filament_ramming_parameters", "", [this](wxWindow* parent) {
            ...
            UpdateDarkUI(ramming_dialog_btn);
            ramming_dialog_btn->SetFont(Slic3r::GUI::app_normal_font());
...
//	auto font = (new wxSystemSettings)->GetFont(wxSYS_DEFAULT_GUI_FONT);
    (*StaticText)->SetFont(app_normal_font());
...
void TabPrinter::build_fff()
{
    ...
    // BBS
    //AppAdapter::obj_list()->update_objects_list_filament_column(m_initial_extruders_count);
    ...
    auto optgroup = page->new_optgroup(L("Printable space"), "param_printable_space");
        ...
        //add 2025
        //optgroup->append_single_option_line("enable_extruder");
        //add end 
        ...
        optgroup->append_single_option_line("hot_bed_divide");
        optgroup->append_single_option_line("idex_mode");
...
void TabPrinter::extruders_count_changed(size_t extruders_count)
{
    ...
    if (is_count_changed) {
        ...
        // BBS
        //AppAdapter::obj_list()->update_objects_list_filament_column(extruders_count);
    }
}
    ...
    for (auto extruder_idx = m_extruders_count_old; extruder_idx < m_extruders_count; ++extruder_idx) {
        ...
        if (!page_exist)
        {
            ...
            m_pages.insert(m_pages.begin() + n_before_extruders + extruder_idx, page);
                ...
                optgroup->append_single_option_line("enable_extruder", "", extruder_idx);
...
void TabPrinter::update_pages()
{
    ...
    if (new_printer_technology == ptFFF)
    {
        if (m_pages_fff.empty())
        {
         ...
         AppAdapter::obj_list()->update_objects_list_filament_column(m_extruders_count);
    }
    else
...
void TabPrinter::toggle_options()
{
    ...
    if (m_active_page->title() == L("Basic information")) {
        ...
        auto hot_bed_divide = m_config->option<ConfigOptionEnum<BedDivide>>("hot_bed_divide")->value;
        //disable idex_mode if hot_bed_divide is One_Area
        if (hot_bed_divide == BedDivide::One_Area) {
            toggle_line("idex_mode", false);
        }
        else {
            toggle_line("idex_mode", true);
        }
    }
    ...
    if (m_active_page->title() == L("Multimaterial")) {
        ...
        toggle_option("purge_in_prime_tower", bSEMM && true);
    }
...
void Tab::load_current_preset()
{
    ...
    if (m_type == Slic3r::Preset::TYPE_PRINTER) {
        ...
        if (preset.printer_technology() == ptFFF)
            ...
            AppAdapter::obj_list()->update_objects_list_filament_column(1);
    }
...

    {
        ...
        if (!AppAdapter::gui_app()->checked_tab(this))
            return;
        update_tab_ui();
            ...
            if (printer_technology != static_cast<TabPrinter*>(this)->m_printer_technology)
            {
                ...
                for (auto tab : AppAdapter::gui_app()->tabs_list) {
                    if (tab->type() == Preset::TYPE_PRINTER) { // Printer tab is shown every time
                        int cur_selection = AppAdapter::gui_app()->tab_panel()->GetSelection();
                        if (cur_selection != 0)
                            AppAdapter::gui_app()->tab_panel()->SetSelection(AppAdapter::gui_app()->tab_panel()->GetPageCount() - 1);
                        continue;
                    }
                    if (tab->supports_printer_technology(printer_technology))
                    {
                        ...
                        if (true) {
                            ...
                            dynamic_cast<Notebook*>(AppAdapter::gui_app()->tab_panel())->InsertPage(AppAdapter::gui_app()->tab_panel()->FindPage(this), tab, tab->title(), bmp_name);
                        }
                        else
#endif
                            AppAdapter::gui_app()->tab_panel()->InsertPage(AppAdapter::gui_app()->tab_panel()->FindPage(this), tab, tab->title(), "");
                        #ifdef __linux__ // the tabs apparently need to be explicitly shown on Linux (pull request #1563)
                            int page_id = AppAdapter::gui_app()->tab_panel()->FindPage(tab);
                            AppAdapter::gui_app()->tab_panel()->GetPage(page_id)->Show(true);
                        #endif // __linux__
                    }
                    else {
                        int page_id = AppAdapter::gui_app()->tab_panel()->FindPage(tab);
                        AppAdapter::gui_app()->tab_panel()->GetPage(page_id)->Show(false);
                        AppAdapter::gui_app()->tab_panel()->RemovePage(page_id);
                    }
                }
                static_cast<TabPrinter*>(this)->m_printer_technology = printer_technology;
                m_active_page = tmp_page;
#ifdef _MSW_DARK_MODE
                dynamic_cast<Notebook*>(AppAdapter::gui_app()->tab_panel())->SetPageImage(AppAdapter::gui_app()->tab_panel()->FindPage(this), printer_technology == ptFFF ? "printer" : "sla_printer");
#endif
            }
            on_presets_changed();
    ...
    if (canceled) {
        ...
        if (current_dirty || delete_current || print_tab || printer_tab)
            m_preset_bundle->update_compatible(
            	update_compatible_type(technology_changed, print_tab,   (print_tab ? this : AppAdapter::gui_app()->get_tab(Preset::TYPE_PRINT))->m_show_incompatible_presets),
            	update_compatible_type(technology_changed, false, 		AppAdapter::gui_app()->get_tab(Preset::TYPE_FILAMENT)->m_show_incompatible_presets));
        // Initialize the UI from the current preset.
        if (printer_tab)
        ...
        if (m_type == Preset::TYPE_PRINTER && AppAdapter::app_config()->get_bool("remember_printer_config")) {
            m_preset_bundle->update_selections(*AppAdapter::app_config());
            AppAdapter::plater()->sidebar().on_filaments_change(m_preset_bundle->filament_presets.size());
        }
        load_current_preset();


        if (delete_third_printer) {
            AppAdapter::app()->CallAfter([filament_presets, process_presets]() {
...
bool Tab::may_discard_current_dirty_preset(PresetCollection* presets /*= nullptr*/, const std::string& new_printer_name /*= ""*/, bool no_transfer)
{
    ...
    else if (dlg.transfer_changes()) // move selected changes
    {
        ...
        if (m_type == presets->type()) // move changes for the current preset from this tab
        {
            ...
            AppAdapter::gui_app()->get_tab(presets->type())->cache_config_diff(selected_options);
    }
...
bool Tab::update_current_page_in_background(int& item)
{
    ...
    if (AppAdapter::main_panel() != nullptr && AppAdapter::main_panel()->is_active_and_shown_tab(m_parent))
        active_tab = true;
...
{
    AppAdapter::main_panel()->diff_dialog.show(m_type);
}
...
//!	m_tabctrl->OnSetFocus();
    ...
    if (m_presets->type() == Preset::TYPE_FILAMENT) {
        _current_printer = const_cast<Preset*>(&app_preset_bundle()->printers.get_selected_preset_base());
    }
    ...
    if (!exist_preset) {
        AppAdapter::plater()->sidebar().update_presets_from_to(m_type, curr_preset_name, new_preset->name);
    }
...

    {
        ...
        for (Preset::Type preset_type : dependent)
            AppAdapter::gui_app()->get_tab(preset_type)->update_tab_ui();
    }

    // update preset comboboxes in DiffPresetDlg
    AppAdapter::main_panel()->diff_dialog.update_presets(m_type);
}
...
wxSizer* Tab::compatible_widget_create(wxWindow* parent, PresetDependencies &deps)
{
    ...
    deps.checkbox->SetFont(Slic3r::GUI::app_normal_font());
    UpdateDarkUI(deps.checkbox, false, true);
    deps.btn = new ScalableButton(parent, wxID_ANY, "printer", from_u8((boost::format(" %s %s") % _utf8(L("Set")) % std::string(dots.ToUTF8())).str()),
                                  wxDefaultSize, wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT, true);
    deps.btn->SetFont(Slic3r::GUI::app_normal_font());
...
wxSizer* TabPrinter::create_bed_shape_widget(wxWindow* parent)
{
    ...
    {
        Search::OptionsSearcher& searcher = AppAdapter::gui_app()->sidebar().get_searcher();
...
void Page::activate(ConfigOptionMode mode, std::function<void()> throw_if_canceled)
{
    ...
    if (m_page_title == NULL) {
        m_page_title = new Label(Font::Head_18, _(m_title), m_parent);
