...
void Selection::add(unsigned int volume_idx, bool as_single_selection, bool check_for_already_contained)
{
    ...
    if (!already_contained || needs_reset) {
        AppAdapter::plater()->take_snapshot(std::string("Selection-Add!"), UndoRedo::SnapshotType::Selection);
...
void Selection::remove(unsigned int volume_idx)
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Selection-Remove!"), UndoRedo::SnapshotType::Selection);
...
void Selection::add_object(unsigned int object_idx, bool as_single_selection)
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Selection-Add Object"), UndoRedo::SnapshotType::Selection);
...
void Selection::remove_object(unsigned int object_idx)
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Selection-Remove Object"), UndoRedo::SnapshotType::Selection);
...
void Selection::add_instance(unsigned int object_idx, unsigned int instance_idx, bool as_single_selection)
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Selection-Add Instance"), UndoRedo::SnapshotType::Selection);
...
void Selection::remove_instance(unsigned int object_idx, unsigned int instance_idx)
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Selection-Remove Instance"), UndoRedo::SnapshotType::Selection);
...
void Selection::add_curr_plate()
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Selection-Add Curr Plate All!"));
    m_mode = Instance;
    clear();
...
void Selection::remove_curr_plate()
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Selection-Delete Curr Plate All"));
    m_mode = Instance;
    clear();
...
void Selection::clone(int numbers)
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Selection-clone"));
    copy_to_clipboard();
...
{
    PartPlate* plate = AppAdapter::plater()->get_partplate_list().get_selected_plate();
    ...
    AppAdapter::plater()->canvas_facade()->do_move(L("Move Object"));
    return;
}
...
void Selection::set_printable(bool printable)
{
    ...
    AppAdapter::plater()->take_snapshot(snapshot_text);
    ...
    for (const std::pair<int, int>& i : instances_idxs)
    {
        ...
        AppAdapter::obj_list()->update_printable_state(i.first, i.second);

        //update printable state on canvas
        AppAdapter::plater()->canvas3D()->update_instance_printable_state_for_object((size_t)i.first);
    }
...
void Selection::add_all()
{
    ...
    AppAdapter::plater()->take_snapshot(std::string("Selection-Add All!"), UndoRedo::SnapshotType::Selection);
...
//    if (!AppAdapter::plater()->can_redo())
        AppAdapter::plater()->take_snapshot(std::string("Selection-Remove All!"), UndoRedo::SnapshotType::Selection);
...
const std::pair<BoundingBoxf3, Transform3d>& Selection::get_bounding_box_in_current_reference_system() const
{
    ...
    ECoordinatesType coordinates_type = AppAdapter::gui_app()->obj_manipul()->get_coordinates_type();
...
void Selection::scale_to_fit_print_volume(const BuildVolume& volume)
{
    auto fit = [this](double s, Vec3d offset) {
        ...
        AppAdapter::plater()->take_snapshot(std::string("Scale To Fit"));
        ...
        AppAdapter::plater()->canvas_facade()->do_scale(""); // avoid storing another snapshot
        ...
        AppAdapter::plater()->canvas_facade()->do_move(""); // avoid storing another snapshot
    };
...
void Selection::erase()
{
    ...
    if (is_single_full_object())
        AppAdapter::obj_list()->delete_from_model_and_list(ItemType::itObject, get_object_idx(), 0);
        ...
        AppAdapter::obj_list()->delete_from_model_and_list(items);
    }
        ...
        AppAdapter::obj_list()->delete_from_model_and_list(ItemType::itInstance, get_object_idx(), get_instance_idx());
    else if (is_mixed()) {
...
//void Selection::render_sidebar_hints(const std::string& sidebar_field)
{
    ...
    GLShaderProgram* shader = get_shader(boost::starts_with(sidebar_field, "layer") ? "flat" : "gouraud_light");
    if (shader == nullptr)
        return;
...
{
    AppAdapter::obj_list()->set_extruder_for_selected_items(extruder_id);
}
...
void Selection::render_synchronized_volumes()
{
    ...
    const ECoordinatesType coordinates_type = AppAdapter::gui_app()->obj_manipul()->get_coordinates_type();
    BoundingBoxf3 box;
    Transform3d trafo;
...
void Selection::render_sidebar_layers_hints(const std::string& sidebar_field, GLShaderProgram& shader)
{
    ...
    const bool camera_on_top = AppAdapter::plater()->get_camera().is_looking_downward();
...
//        bool               mirrored = curr_inst_trafo_i.linear().determinant() * old_inst_trafo_i.linear().determinant() < 0;
        ...
        for (unsigned int j = 0; j < (unsigned int)m_volumes->size(); ++j) {
            ...
            if (app_preset_bundle()->printers.get_edited_preset().printer_technology() != ptSLA)
...
//                double offset = AppAdapter::plater()->canvas3D()->get_size_proportional_to_max_bed_size(0.05);
//                dst_volume->translate(dst_matrix.inverse() * Vec3d(offset, offset, 0.0));
        ...
        AppAdapter::obj_list()->paste_volumes_into_list(dst_obj_idx, volumes);
    }
...
void Selection::paste_objects_from_clipboard()
{
    ...
    for (size_t i=0;i<src_objects.size();i++)
    {
        ...
        if (shift_all(0) != 0 || shift_all(1) != 0) {
            // BBS: if multiple objects are selected, move them as a whole after copy
            if (i == 0) empty_cell_all = AppAdapter::plater()->canvas3D()->get_nearest_empty_cell({start_point(0), start_point(1)}, {bbox.size()(0)+1,bbox.size()(1)+1});
            ...
            auto empty_cell   = AppAdapter::plater()->canvas3D()->get_nearest_empty_cell({start_point(0), start_point(1)}, {bbox.size()(0)+1, bbox.size()(1)+1});
    ...
    AppAdapter::obj_list()->paste_objects_into_list(object_idxs);
