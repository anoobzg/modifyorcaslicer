...
std::vector<Slic3r::ColorRGBA> get_extruders_colors()
{
    ...
    std::vector<std::string>          colors        = Slic3r::GUI::AppAdapter::plater()->get_extruder_colors_from_plater_config();
...
void GLVolume::SinkingContours::render()
{
    ...
    GLShaderProgram* shader = GUI::get_current_shader();
    if (shader == nullptr)
        return;

    const GUI::Camera& camera = GUI::AppAdapter::plater()->get_camera();
...
void GLVolume::SinkingContours::update()
{
    ...
    const Model& model = GUI::AppAdapter::plater()->model();
...
void glvolume_update_render_colors()
{
...
void glvolume_load_render_colors()
{
...
Transform3d GLVolume::world_mirror_matrix() const
{
    Transform3d world_matrix = this->world_matrix();
    world_matrix.translation() = m_mirror_offset;
    return world_matrix; //   返回镜像
}
...
{
    return GUI::AppAdapter::plater()->model().objects[object_idx()]->volumes[volume_idx()]->mesh().transformed_bounding_box(trafo, 0.0);
}
...
void GLVolume::render_with_outline(const GUI::Size& cnv_size)
{
    ...
    if (Slic3r::GUI::is_unkown_framebuffer()) {
    ...
    GLuint depth_fbo   = 0;
    GLuint depth_tex = 0;
    if (Slic3r::GUI::is_arb_framebuffer()) {
    ...
    } else if (Slic3r::GUI::is_ext_framebuffer()) {
...
{
    if (is_modifier || GUI::app_preset_bundle()->printers.get_edited_preset().printer_technology() == ptSLA)
        return false;
    ...
    float rotation_angle, bool size_unknown, float brim_width)
{
    ...
    GUI::PartPlateList& ppl = GUI::AppAdapter::plater()->get_partplate_list();
...
{
    const DynamicPrintConfig& glb_cfg        = GUI::app_preset_bundle()->prints.get_edited_preset().config;
...
void GLVolumeCollection::render(GLVolumeCollection::ERenderType type, bool disable_cullface, const Transform3d& view_matrix, const Transform3d& projection_matrix, const GUI::Size& cnv_size,
    std::function<bool(const GLVolume&)> filter_func, ERenderMode mode) const
{
    ...
    GLShaderProgram* sink_shader = GUI::get_shader("flat");
    GLShaderProgram* edges_shader = GUI::get_shader("flat");
    ...
    bool need_render_slave = (mode == ERenderMode::Mirror || mode == ERenderMode::Copy);
    auto render_volumes = [&](bool render_slave, ERenderMode mode) 
    {
        for (GLVolumeWithIdAndZ& volume : to_render) {

            Transform3d worldMatix ;
            worldMatix = volume.first->world_matrix();

            if (render_slave)
            {
                if (volume.first->is_outside)
                    continue;
                
                if (mode == ERenderMode::Mirror)
                {
                    Matrix4d mirrorMatrix;
                    mirrorMatrix.setIdentity();
                    mirrorMatrix(0,0) = 1.0;
                    mirrorMatrix(1,1) = -1.0;
                    mirrorMatrix(2,2) = 1.0;
                    mirrorMatrix(1,3) =  2.0*volume.first->mirror_center.y();
                    Transform3d temp ;
                    temp = mirrorMatrix;
                    worldMatix = temp*volume.first->world_matrix();     
                }
                else if (mode == ERenderMode::Copy)
                {
                    Vec3d offset = volume.first->world_matrix().translation();
                    Vec3d mirror_offset = offset + volume.first->copy_offset;
                    volume.first->set_mirror_offset(mirror_offset);
                    worldMatix =  volume.first->world_mirror_matrix();
                }
            }
            ...
            shader->set_uniform("volume_world_matrix", worldMatix);
            shader->set_uniform("slope.actived", m_slope.isGlobalActive && !volume.first->is_modifier && !volume.first->is_wipe_tower);
            shader->set_uniform("slope.volume_world_normal_matrix", static_cast<Matrix3f>(worldMatix.matrix().block(0, 0, 3, 3).inverse().transpose().cast<float>()));
            shader->set_uniform("slope.normal_z", normal_z);

    #if ENABLE_ENVIRONMENT_MAP
            unsigned int environment_texture_id = GUI::AppAdapter::plater()->get_environment_texture_id();
            bool use_environment_texture = environment_texture_id > 0 && app_get_bool("use_environment_map");
            ...
            const Transform3d model_matrix = worldMatix;
            shader->set_uniform("view_model_matrix", view_matrix * model_matrix);
            shader->set_uniform("projection_matrix", projection_matrix);
            const Matrix3d view_normal_matrix = view_matrix.matrix().block(0, 0, 3, 3) * model_matrix.matrix().block(0, 0, 3, 3).inverse().transpose();
            shader->set_uniform("view_normal_matrix", view_normal_matrix);
		    //BBS: add outline related logic
            if (volume.first->selected && Slic3r::GUI::show_outline())
    ...
    render_volumes(false, mode);
    if (need_render_slave)
        render_volumes(true, mode);
    ...
    //    {
    //    //     Vec3d offset = volume->world_matrix().translation();
    //    //     Vec3d mirror_offset = mirrorPoint3D(offset, Vec3d(0, 1, 0), center);

    //    //     volume->update_mirror_offset(mirror_offset); // 这种镜像
    //}
}


bool GLVolumeCollection::check_outside_state(const BuildVolume& plate_build_volume, ModelInstanceEPrintVolumeState* out_state) const
{
    if (GUI::AppAdapter::plater() == NULL)
    {
    ...
    for (GLVolume* volume : this->volumes)
    {
        if (!volume->is_modifier && (volume->shader_outside_printer_detection_enabled || (!volume->is_wipe_tower && volume->composite_id.volume_id >= 0))) {
            ...
            volume->is_outside = !(!volume->is_outside || state == BuildVolume::ObjectState::Inside);
            if (volume->printable) {
