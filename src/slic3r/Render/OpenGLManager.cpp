...
{
    // Initialize the shaders manager.
    m_shaders_manager = std::make_unique<GLShadersManager>();
}
...
bool OpenGLManager::init_gl(bool popup_error)
{
	...
	//BOOST_LOG_TRIVIAL(info) << "glewInit Success."<< std::endl;
        ...
        m_shaders_init_result = m_shaders_manager->init();
#ifdef _WIN32
...
bool OpenGLManager::is_initialized()
{
    return m_gl_initialized;
}

bool OpenGLManager::init_shaders_happen_error(std::string& error)
{
    if(!m_shaders_init_result.first)
        error = m_shaders_init_result.second;
    return !m_shaders_init_result.first;
}
...
int* OpenGLManager::get_shared_attrib_list()
{
    if (s_multisample == EMultisampleState::Unknown) {
        detect_multisample(shared_attribList);
    }

    if (! can_multisample())
        shared_attribList[12] = 0;

    return shared_attribList;
}

wxGLContext* OpenGLManager::get_shared_context(wxGLCanvas& canvas)
{
    return init_glcontext(canvas);
}
...
GLShaderProgram* OpenGLManager::get_shader(const std::string& shader_name)
{ 
    return m_shaders_manager->get_shader(shader_name); 
}

GLShaderProgram* OpenGLManager::get_current_shader() 
{ 
    return m_shaders_manager->get_current_shader(); 
}
