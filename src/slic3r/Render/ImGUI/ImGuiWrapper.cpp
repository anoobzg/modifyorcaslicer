...
void ImGuiWrapper::render_draw_data(ImDrawData *draw_data)
{
    ...
    GLShaderProgram* shader = get_shader("imgui");
    if (shader == nullptr)
        return;
    ...
    GLShaderProgram* curr_shader = get_current_shader();
...
std::unique_ptr<ImGuiWrapper> m_imgui(new ImGuiWrapper());

void init_imgui(const std::string &language)
{
    m_imgui->set_language(language);
}

ImGuiWrapper& global_im_gui()
{
    return *m_imgui;
}
