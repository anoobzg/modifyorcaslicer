...
static std::string detect_attached_device()
{
    ...
    else if (app_get_bool("skip_3dmouse_detect")) {
...
void Mouse3DController::render_settings_dialog(GLCanvas3D& canvas) const
{
    ...
    ImGuiWrapper::push_toolbar_style(AppAdapter::plater()->canvas3D()->get_scale());
    ...
    if (imgui.begin(_L("3Dconnexion settings"), &shown, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse || ImGuiWindowFlags_NoTitleBar)) {
        if (shown) {
            ...
            Vec3f target = AppAdapter::plater()->get_camera().get_target().cast<float>();
...
void Mouse3DController::disconnected()
{
    ...
    if (m_connected) {
	    ...
	    m_device_str.clear();
	    m_connected = false;
		get_notification_manager()->push_notification(NotificationType::Mouse3dDisconnected);

        AppAdapter::plater()->CallAfter([]() {
...
{
    if (! AppAdapter::gui_app()->IsActive())
        return false;
...
{
    if (m_stop || app_get_bool("skip_3dmouse_detect"))
    	return false;
