...
protected:
    ...
    Geometry::Transformation m_mirror_transformation;  // 镜像的偏移
    // BBS
...
public:
    ...
    void set_mirror_offset(Vec3d mirror) { m_mirror_offset = mirror;}
    ...
    Transform3d         world_mirror_matrix() const ;
...
public:
    ...
    enum class ERenderMode : unsigned char
    {
        Normal,
        Pack,
        Mirror,
...
public:
    ...
    void render(ERenderType type, bool disable_cullface, const Transform3d& view_matrix, const Transform3d& projection_matrix, const GUI::Size& cnv_size,
                std::function<bool(const GLVolume &)> filter_func  = std::function<bool(const GLVolume &)>(), ERenderMode mode = ERenderMode::Normal) const;
    ...
    bool check_outside_state(const BuildVolume& plate_build_volume, ModelInstanceEPrintVolumeState* out_state) const;
