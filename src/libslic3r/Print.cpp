...
std::string Print::output_filename(const std::string &filename_base) const
{
    ...
    std::string normal_print_time = short_time(m_print_statistics.estimated_normal_print_time);
    std::string silent_print_time = short_time(m_print_statistics.estimated_silent_print_time);
    ...
    config.set_key_value("initial_tool", new ConfigOptionInt((int)m_print_statistics.initial_tool));
        ...
        [filename_vector, &json_vector, with_space, &ret, &mutex](const tbb::blocked_range<size_t>& output_range) {
            for (size_t object_index = output_range.begin(); object_index < output_range.end(); ++ object_index) {
                try {
                    ...
                    if (!c.is_open()) {
                        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": failed to open file " << filename_vector[object_index];
                        break;
                    }
                    if (with_space)
