...
std::string AppConfig::load()
{
    ...
    BOOST_LOG_TRIVIAL(info) << format("Semver version %1% = %2%.%3%.%4%", get("version").c_str(),
                             m_orig_version.maj(), m_orig_version.min(), m_orig_version.patch());
...
void AppConfig::save()
{
    ...
    try {
        ...
        if (!c.is_open()) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": failed to open file " << path_pid;
            return;
        }
        ...
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": save to " << path_pid << " got a generic exception, reason = " << err.what();
        return;
    }
...
std::string AppConfig::config_path()
{
    ...
    std::string path = (boost::filesystem::path(Slic3r::user_data_dir()) / (boost::format("%1%.conf")%SLIC3R_APP_KEY).str()).make_preferred().string();
#else
    std::string path = (boost::filesystem::path(Slic3r::user_data_dir()) / (boost::format("%1%.ini")%SLIC3R_APP_KEY).str()).make_preferred().string();
#endif
