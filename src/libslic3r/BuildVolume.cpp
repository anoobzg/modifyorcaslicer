...
BuildVolume::BuildVolume(const std::vector<std::vector<Vec2d>>& printable_area, const double printable_height)
    :m_bed_shape(printable_area), m_max_print_height(printable_height)
{
    assert(printable_height >= 0);

    std::vector<Vec2d>  printable_area_combine;
    if (printable_area.size() == 4)
    {
        printable_area_combine = printable_area.front();
        int i = 0;
        for (auto& shape : printable_area)
        {
            printable_area_combine.push_back(shape[i++]);
        }
    }
    else 
    {
    ...
    m_polygon     = Polygon::new_scale(printable_area_combine);
    ...
    BoundingBoxf bboxf = get_extents(printable_area_combine);
    m_bboxf = BoundingBoxf3{ to_3d(bboxf.min, 0.), to_3d(bboxf.max, printable_height) };

    if (printable_area_combine.size() >= 4 && std::abs((m_area - double(m_bbox.size().x()) * double(m_bbox.size().y()))) < sqr(SCALED_EPSILON)) {
    ...
    } else if (printable_area_combine.size() > 3) {
...
//        m_circle = Geometry::circle_taubin_newton(printable_area);
        m_circle = Geometry::circle_ransac(printable_area_combine);
        bool is_circle = true;
#ifndef NDEBUG
        // Measuring maximum absolute error of interpolating an input polygon with circle.
        double max_error = 0;
#endif // NDEBUG
        Vec2d prev = printable_area_combine.back();
        for (const Vec2d &p : printable_area_combine) {
#ifndef NDEBUG
    ...
    if (printable_area_combine.size() >= 3 && m_type == BuildVolume_Type::Invalid) {
