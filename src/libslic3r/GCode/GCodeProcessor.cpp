...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    ...
    // Support for LightMaker generated gcode (compatible with OrcaSlicer format)
    { EProducer::OrcaSlicer, "generated by LightMaker" }
...
{
    if (process_lightmaker_tags(comment))
        return;
...
bool GCodeProcessor::process_lightmaker_tags(const std::string_view comment)
{
    if (boost::starts_with(comment, " idex_mode = "))
    {
        auto data = comment.substr(strlen(" idex_mode = "));
        if (data == "Copy")
            this->m_result.idex_mode = IdexMode_Copy;
        else if (data == "Mirror")
            this->m_result.idex_mode = IdexMode_Mirror;
        else 
            this->m_result.idex_mode = IdexMode_Pack;

        return true;
    }
    else if (boost::starts_with(comment, " hot_bed_divide = "))
    {
        auto data = comment.substr(strlen(" hot_bed_divide = "));
        if (data == "Four_Areas")
            this->m_result.hot_bed_divide = Four_Areas;
        else 
            this->m_result.hot_bed_divide = One_Area;
    }
