...
std::string debug_out_path(const char *name, ...)
{
	...
	auto svg_folder = boost::filesystem::path(user_data_dir()) / "SVG/";
...
{
	return (boost::format("%1%%2%")%SLIC3R_APP_NAME%LightMaker_VERSION).str();
}

std::string header_gcodeviewer_generated()
{
	return (boost::format("%1%%2%")%GCODEVIEWER_APP_NAME%LightMaker_VERSION).str();
}
