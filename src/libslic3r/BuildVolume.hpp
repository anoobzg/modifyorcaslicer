...
public:
    ...
    BuildVolume(const std::vector<std::vector<Vec2d>> &printable_area, const double printable_height);

    // Source data, unscaled coordinates.
    const std::vector<std::vector<Vec2d>>&   printable_area()         const { return m_bed_shape; }
...
private:
    ...
    std::vector<std::vector<Vec2d>> m_bed_shape;
