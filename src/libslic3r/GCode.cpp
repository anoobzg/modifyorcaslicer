...
static void force_create_directory(const fs::path& folder)
{
    if (fs::exists(folder))
        return;
    force_create_directory(folder.parent_path());
...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if (print.config().gcode_flavor == gcfMarlinLegacy || print.config().gcode_flavor == gcfMarlinFirmware) {
        if (print.config().idex_mode == IdexMode::IdexMode_Copy) {
            file.write_format("M605 S2 \n");
        }
        else if (print.config().idex_mode == IdexMode::IdexMode_Mirror) {
            file.write_format("M605 S3 \n");
        }
    }
    else if (print.config().gcode_flavor == gcfKlipper) {
        if (print.config().idex_mode == IdexMode::IdexMode_Copy) {
            file.write_format("SET_DUAL_CARRIAGE MODE=COPY \n");
        }
        else if (print.config().idex_mode == IdexMode::IdexMode_Mirror) {
            file.write_format("SET_DUAL_CARRIAGE MODE=MIRROR \n");
        }
    }
