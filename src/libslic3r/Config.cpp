...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    bool has_delimiters = true;
    {
        ...
        // Support for LightMaker generated gcode files
        std::string lightmaker_gcode_header = "; generated by LightMaker";
        ...
        while (std::getline(ifs, header)) {
            ...
            // BBS - Check for OrcaSlicer, BambuStudio, or LightMaker headers
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                ...
                strncmp(lightmaker_gcode_header.c_str(), line_c, strlen(lightmaker_gcode_header.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
        if (!header_found) {
            ...
            error_message += " or compatible slicers.";
...
void ConfigBase::save_to_json(const std::string &file, const std::string &name, const std::string &from, const std::string &version, const std::string is_custom) const
{
    ...
    try {
        ...
        if (!c.is_open()) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": failed to open file " << file;
            return;
        }
        ...
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": save to " << file << " got a generic exception, reason = " << err.what();
        return;
    }
