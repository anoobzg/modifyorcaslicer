...
static std::vector<std::string> s_Preset_print_options {
    ...
    "flush_into_infill", "flush_into_objects", "flush_into_support",
     ...
     "wipe_tower_cone_angle", "wipe_tower_extra_spacing","wipe_tower_max_purge_speed", "wipe_tower_filament", "wiping_volumes_extruders","wipe_tower_bridging", "wipe_tower_extra_flow","single_extruder_multi_material_priming","hot_bed_divide","idex_mode",
...
static std::vector<std::string> s_Preset_printer_options {
    "printer_technology",
    "printable_area", "bed_exclude_area","bed_custom_texture", "bed_custom_model", "gcode_flavor", "idex_mode", "hot_bed_divide",
...
namespace PresetUtils {
    ...
    std::string system_printer_bed_model(const Preset& preset)
    {
        ...
        if (pm != nullptr && !pm->bed_model.empty()) {
            out = Slic3r::user_data_dir() + "/vendor/" + preset.vendor->id + "/" + pm->bed_model;
    ...
    std::string system_printer_bed_texture(const Preset& preset)
    {
        ...
        if (pm != nullptr && !pm->bed_texture.empty()) {
            out = Slic3r::user_data_dir() + "/vendor/" + preset.vendor->id + "/" + pm->bed_texture;
            if (!boost::filesystem::exists(boost::filesystem::path(out)))
    ...
    std::string system_printer_hotend_model(const Preset& preset)
    {
        ...
        if (pm != nullptr && !pm->hotend_model.empty()) {
            out = Slic3r::user_data_dir() + "/vendor/" + preset.vendor->id + "/" + pm->hotend_model;
            if (!boost::filesystem::exists(boost::filesystem::path(out)))
        ...
        if (out.empty() ||!boost::filesystem::exists(boost::filesystem::path(out)))
            out = Slic3r::resources_dir() + "/profiles_save/hotend.stl";
        return out;
    }
} // namespace PresetUtils
