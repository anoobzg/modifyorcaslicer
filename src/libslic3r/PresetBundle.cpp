...
{
    boost::filesystem::path data_dir = boost::filesystem::path(Slic3r::user_data_dir());
...
{
    const std::string backup_folderpath = user_data_dir() + "/" + (boost::format("user_backup-v%1%") % LightMaker_VERSION).str();
    ...
    try {
        ...
        boost::filesystem::copy(user_data_dir() + "/" + PRESET_USER_DIR, backup_folderpath, boost::filesystem::copy_options::recursive);
...
std::string PresetBundle::get_texture_for_printer_model(std::string model_name)
{
    ...
    {
        out = Slic3r::user_data_dir() + "/vendor/" + vendor_name + "/" + texture_name;
...
std::string PresetBundle::get_stl_model_for_printer_model(std::string model_name)
{
    ...
    if (!stl_name.empty())
    {
        out = Slic3r::user_data_dir() + "/vendor/" + vendor_name + "/" + stl_name;
        if (!boost::filesystem::exists(boost::filesystem::path(out)))
...
std::string PresetBundle::get_hotend_model_for_printer_model(std::string model_name)
{
    ...
    if (!hotend_stl.empty())
    {
        out = Slic3r::user_data_dir() + "/vendor/" + vendor_name + "/" + hotend_stl;
        if (!boost::filesystem::exists(boost::filesystem::path(out)))
...
PresetsConfigSubstitutions PresetBundle::load_user_presets(std::string user, ForwardCompatibilitySubstitutionRule substitution_rule)
{
    ...
    fs::path user_folder(user_data_dir() + "/" + PRESET_USER_DIR);
    if (!fs::exists(user_folder)) fs::create_directory(user_folder);

    std::string dir_user_presets = user_data_dir() + "/" + PRESET_USER_DIR + "/" + user;
                                                        ...
                                                        ForwardCompatibilitySubstitutionRule    rule)
{
    ...
    for (auto &file : files) {
        ...
        if (boost::iends_with(file, ".orca_printer") || boost::iends_with(file, ".orca_filament") || boost::iends_with(file, ".zip")) {
            ...
            if (ec) BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << " create directory failed: " << ec.message();
            //create temp folder
            //std::string user_default_temp_dir = user_data_dir() + "/" + PRESET_USER_DIR + "/" + DEFAULT_USER_FOLDER_NAME + "/" + "temp";
...
void PresetBundle::save_user_presets(AppConfig& config, std::vector<std::string>& need_to_delete_list)
{
    ...
    const std::string dir_user_presets = user_data_dir() + "/" + PRESET_USER_DIR + "/"+ user_sub_folder;
...
{
    //BBS: change directory by design
    const std::string dir_user_presets = user_data_dir() + "/" + PRESET_USER_DIR + "/"+ preset_folder;
...
std::pair<PresetsConfigSubstitutions, std::string> PresetBundle::load_system_presets_from_json(ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    //BBS: change directory by design
    boost::filesystem::path     dir = (boost::filesystem::path(user_data_dir()) / PRESET_SYSTEM_DIR).make_preferred();
    if (validation_mode)
        dir = (boost::filesystem::path(user_data_dir())).make_preferred();
    ...
    const std::string &path, const std::string &vendor_name, LoadConfigBundleAttributes flags, ForwardCompatibilitySubstitutionRule compatibility_rule, const PresetBundle* base_bundle)
{
    ...
    auto parse_subfile = [this, path, vendor_name, presets_loaded, current_vendor_profile, base_bundle](
        ...
        auto file_path = (boost::filesystem::path(user_data_dir())  /PRESET_SYSTEM_DIR/ vendor_name / subfile_iter.second).make_preferred();
        if(validation_mode)
            file_path = (boost::filesystem::path(user_data_dir()) / vendor_name / subfile_iter.second).make_preferred();
