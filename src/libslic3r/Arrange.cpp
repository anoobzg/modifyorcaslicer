...
Points get_shrink_bedpts(const DynamicPrintConfig* print_cfg, const ArrangeParams& params, const Points& _bedpts)
{
    Points bedpts = _bedpts.empty() ? get_bed_shape(*print_cfg) : _bedpts;
...
Points get_plate_shape(const std::vector<Pointfs>& plate_shape, const Point& center)
{
    Points plate_shape_points;
    for (const auto& shape : plate_shape) {
        Polygon polygon;
        polygon.points.reserve(shape.size());
        for (const auto& point : shape) {
            polygon.points.emplace_back(scaled<coord_t>(point));
        }

        if (polygon.contains(center)) {
            plate_shape_points = polygon.points;
            break;
        }
    }

    if (plate_shape_points.empty() && !plate_shape.empty()) {
        plate_shape_points.reserve(plate_shape.front().size());
        for (const auto& point : plate_shape.front()) {
            plate_shape_points.emplace_back(scaled<coord_t>(point));
        }
    }
    return plate_shape_points;
}
